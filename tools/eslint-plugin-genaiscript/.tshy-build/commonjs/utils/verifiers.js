...
exports.getVerifiers = exports.arrayToString = exports.stripFileName = exports.stripPath = exports.VerifierMessages = void 0;
exports.isEsmPackage = isEsmPackage;
const node_fs_1 = require("node:fs");
const node_path_1 = __importDefault(require("node:path"));
exports.VerifierMessages = {
    ...
    arrayContainsNonLiteral: "{{array}} contains non-literal (string | boolean | null | number | RegExp) elements",
 ...
 */
const stripPath = (pathOrFileName) => pathOrFileName.replace(/^.*[\\\/]/, "");
...
function isEsmPackage(filePath) {
    const packageJsonPath = filePath.endsWith("package.json")
        ? filePath
        : node_path_1.default.join(node_path_1.default.dirname(filePath), "package.json");
    try {
 ...
 */
const stripFileName = (pathOrFileName) => pathOrFileName.replace(/[\\\/][^\\\/]+$/, "");
 ...
 */
const arrayToString = (array) => JSON.stringify(array).replace(/,/g, ", ");
 ...
 */
const getVerifiers = (context, data) => ({
    /**
    ...
    outerMatchesExpected: (node) => {
        ...
        if (nodeValue.value !== expected) {
            context.report({
                ...
                fix: (fixer) => fixer.replaceText(nodeValue, typeof expected === "string" ? `"${expected}"` : expected),
            });
        }
    },
    /**
    ...
    outerContainsExpected: (node) => {
        ...
        const nodeValue = node.value;
        const nonLiteral = nodeValue.elements.find((element) => element.type !== "Literal");
        ...
        const candidateArray = nodeValue.elements;
        const candidateValues = candidateArray.map((candidate) => candidate.value);
                ...
                if (!candidateValues.includes(value)) {
                    context.report({
                        ...
                        fix: (fixer) => {
                            ...
                            return fixer.replaceText(nodeValue, (0, exports.arrayToString)(candidateValues));
                        },
                    });
                }
            });
...
exports.getVerifiers = getVerifiers;
