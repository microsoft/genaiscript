...
* `system.*.genai.js` sont considérés comme des modèles d'invites système
* Les invites système sont non listées par défaut
* Les invites système doivent utiliser `system` au lieu de `script`
* Les invites système sont exécutées avec le même environnement que l'invite principale
...
:::caution
Les invites système doivent avoir une fonction par défaut et utiliser le `ctx` passé dans la fonction.
:::

Pour utiliser les invites système dans un script, remplissez le champ `system` avec les identifiants des scripts.
...
Il est également possible de peupler le script système en incluant les noms d'outils
...
Le système supporte aussi les paramètres comme un script mais les noms de paramètres seront automatiquement préfixés
...
* remplacez la valeur du paramètre dans le script script
...
* remplacez la valeur du paramètre dans une instance du script système
...
## Invites système automatisées

Lorsqu'elles ne sont pas spécifiées, GenAIScript inspecte le code source du script
...
GenAIScript est livré avec un certain nombre d'invites système qui prennent en charge des fonctionnalités comme la création de fichiers, l'extraction de différences ou
...
Agent capable d'interroger des données dans des fichiers
...
Agent capable d'interroger la documentation.
...
Agent capable de trouver, rechercher ou lire des fichiers pour accomplir des tâches
...
Agent capable d'interroger Git pour accomplir des tâches.
...
Agent capable d'interroger GitHub pour accomplir des tâches.
...
Agent capable d'exécuter des interprètes de code pour Python, Math.
...
Agent du protocole de contexte modèle
...
Agent pouvant poser des questions à l'utilisateur.
...
Agent pouvant traiter des vidéos
...
Agent capable de rechercher sur le web.
...
Agent capable de formaliser et résoudre des problèmes avec Z3.
...
Émet des annotations compatibles avec GitHub Actions

Les workflows GitHub Actions prennent en charge les annotations ([En savoir plus...](https://docs.github.com/en/actions/using-workflows/workflow-commands-for-github-actions#setting-an-error-message)).
...
Invite d'assistant utile.

Une invite pour un assistant utile provenant de [https://medium.com/@stunspot/omni-f3b1934ae0ea](https://medium.com/@stunspot/omni-f3b1934ae0ea).
...
Raisonnement de chaîne de brouillon

Technique de raisonnement chaîne de brouillon. Plus d'informations sur [https://learnprompting.org/docs/intermediate/zero\_shot\_cot](https://learnprompting.org/docs/intermediate/zero_shot_cot).
...
Génère des modifications de formatteur de journal de modifications
...
Principes de coopération de la maxime de Grice.
...
Génère des différences concises de fichiers.
...
Un outil qui peut récupérer des données d'une URL

* outil `fetch` : récupérer des données d'une URL depuis des domaines autorisés.
...
Appliquer des schémas JSON aux données générées.
...
* outil `fs_ask_file` : exécute une requête LLM sur le contenu d'un fichier. Utilisez cet outil pour extraire des informations d'un fichier.
...
Un outil qui peut interroger des données dans un fichier

* outil `fs_data_query` : interroger des données dans un fichier en utilisant la syntaxe GROQ
...
* outil `fs_diff_files` : calcule un diff entre deux fichiers différents. Utilisez git diff pour comparer les versions d'un fichier.
...
* outil `fs_find_files` : trouve des fichiers correspondant à un motif glob. Utilisez pattern pour spécifier une expression régulière à rechercher dans le contenu du fichier. Faites attention à ne pas demander trop de fichiers.
...
* outil `fs_read_file` : lit un fichier comme texte depuis le système de fichiers. Retourne undefined si le fichier n'existe pas.
...
* outil `github_actions_job_logs_get` : télécharge le log de job de workflow github. Si le log est trop grand, utilisez 'github\_actions\_job\_logs\_diff' pour comparer les logs.
...
Outils pour interroger les fichiers GitHub.
...
Outils pour interroger les pull requests GitHub.
...
Outil modifié de meta-invite provenant de [https://platform.openai.com/docs/guides/prompt-generation?context=text-out](https://platform.openai.com/docs/guides/prompt-generation?context=text-out).

* outil `meta_prompt` : outil qui applique les directives de meta-invite d'OpenAI à une invite utilisateur. Modifié de [https://platform.openai.com/docs/guides/prompt-generation?context=text-out](https://platform.openai.com/docs/guides/prompt-generation?context=text-out).
...
Générateur de méta schéma d'OpenAI depuis [https://platform.openai.com/docs/guides/prompt-generation?context=structured-output-schema](https://platform.openai.com/docs/guides/prompt-generation?context=structured-output-schema).

* outil `meta_schema` : génère un schéma JSON valide pour le JSON décrit. Source [https://platform.openai.com/docs/guides/prompt-generation?context=structured-output-schema](https://platform.openai.com/docs/guides/prompt-generation?context=structured-output-schema).
...
Développeur Python qui ajoute des types.
...
Ce script système doit être considéré pour la génération de contenu (qu'elle soit fondée ou non), les chats multi-tours et single-tours, Q\&A, réécriture et scénarios de résumé. Voir [https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/safety-system-message-templates](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/safety-system-message-templates).
...
Ce script système doit être considéré pour des scénarios tels que : génération de contenu (fondée et non fondée), chat multi-tours et single-tours, Q\&A, réécriture, résumé et génération de code. Voir [https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/safety-system-message-templates](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/safety-system-message-templates).
...
Doit être considéré pour des scénarios tels que le résumé. Voir [https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/safety-system-message-templates](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/safety-system-message-templates).
...
L'outil 'think' d'Anthropic défini dans [https://www.anthropic.com/engineering/claude-think-tool](https://www.anthropic.com/engineering/claude-think-tool). Utilise l'alias de modèle 'think'.
...
Technique de chaîne de raisonnement Zero-shot. Plus d'informations sur [https://learnprompting.org/docs/intermediate/zero\_shot\_cot](https://learnprompting.org/docs/intermediate/zero_shot_cot).
