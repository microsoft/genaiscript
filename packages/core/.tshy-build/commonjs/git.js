...
const packagemanagers_js_1 = require("./packagemanagers.js");
const cleaners_js_2 = require("./cleaners.js");
const workdir_js_1 = require("./workdir.js");
const debug_js_1 = require("./debug.js");
const dbg = (0, debug_js_1.genaiscriptDebug)("git");
async function checkDirectoryExists(directory) {
    const stat = await (0, fs_js_1.tryStat)(directory);
    dbg(`directory exists: ${!!stat?.isDirectory()}`);
    return !!stat?.isDirectory();
}
function appendExtras(rest, args) {
    Object.entries(rest)
        .filter(([, v]) => v !== undefined && typeof v !== "object")
        .forEach(([k, v]) => args.push(v === true ? `--${(0, inflection_1.underscore)(k)}` : `--${(0, inflection_1.underscore)(k)}=${v}`));
}
/**
...
class GitClient {
    _cwd;
    git = "git"; // Git command identifier
    _defaultBranch; // Stores the default branch name
    _requiresSafeDirectory = false; // Indicates if the client requires a safe directory
    constructor(cwd) {
    ...
    static default() {
        if (!this._default)
            this._default = new GitClient(undefined);
        return this._default;
    }
    get cwd() {
        return this._cwd;
    }
    setGitHubWorkspace(cwd) {
        if (cwd === this._cwd)
            return this;
    ...
    async configGlobalAddSafeDirectory() {
    ...
    async resolveExcludedPaths(options) {
        dbg(`resolving excluded paths`);
        const { excludedPaths } = options || {};
        const ep = (0, cleaners_js_1.arrayify)(excludedPaths, { filterEmpty: true });
        const dp = (await (0, fs_js_1.tryReadText)(constants_js_1.GIT_IGNORE_GENAI))?.split("\n");
        dbg(`reading GENAISCRIPTIGNORE file`);
        const dp2 = (await (0, fs_js_1.tryReadText)(constants_js_1.GENAISCRIPTIGNORE))?.split("\n");
        const ps = [
            ...(0, cleaners_js_1.arrayify)(ep, { filterEmpty: true }),
            ...(0, cleaners_js_1.arrayify)(dp, { filterEmpty: true }),
            ...(0, cleaners_js_1.arrayify)(dp2, { filterEmpty: true }),
        ];
        return (0, es_toolkit_1.uniq)(ps);
    }
    /**
    ...
    async fetch(remote, branchOrSha, options) {
    ...
    async exec(args, options) {
        ...
        const eargs = Array.isArray(args) ? args : (0, shell_js_1.shellParse)(args);
        dbg(`exec`, (0, shell_js_1.shellQuote)(eargs));
     ...
     */
    async listFiles(scope, options) {
        dbg(`listing files with scope: ${scope}`);
        const runtimeHost = (0, host_js_1.resolveRuntimeHost)();
        scope = scope || "modified";
        const { askStageOnEmpty } = options || {};
        const paths = (0, cleaners_js_1.arrayify)(options?.paths, { filterEmpty: true });
     ...
     * @param args Git command arguments.
     */
    static addFileFilters(paths, excludedPaths, args) {
    ...
    async addWorktree(path, commitish, options) {
    ...
    async log(options) {
        ...
        for (const line of res.split(/\n/g).map((l) => l.trim())) {
            const g = /^(?<sha>[a-z0-9]{6,40}) (?<date>\d{4,4}-\d{2,2}-\d{2,2}) (?<author>.+) (?<message>.*)$/.exec(line)?.groups;
            if (g) {
                commits.push((commit = {
                    sha: g?.sha,
                    date: g?.date,
    ...
    async changedFiles(options) {
        ...
        const files = (0, es_toolkit_1.uniq)(commits.flatMap((commit) => commit.files)).map((filename) => ({ filename }));
        if (readText)
            await (0, file_js_1.resolveFileContents)(files);
        return files;
    }
    ...
    async diff(options) {
        ...
        const paths = (0, cleaners_js_1.arrayify)(options?.paths, { filterEmpty: true });
        const excludedPaths = await this.resolveExcludedPaths(options);
        const { staged, base, head, unified, askStageOnEmpty, nameOnly, maxTokensFullDiff = constants_js_1.GIT_DIFF_MAX_TOKENS, llmify, algorithm = "minimal", extras, } = options || {};
        const args = ["diff"];
        if (staged) {
        ...
        if (!nameOnly && llmify) {
            dbg(`llmifying diff`);
            res = (0, llmdiff_js_1.llmifyDiff)(res);
            dbg(`encoding diff`);
            const tokens = (0, tokens_js_1.approximateTokens)(res);
...
${(0, util_js_1.ellipse)(res, maxTokensFullDiff * 3)}
...
...
`;
    ...
    async shallowClone(repository, options) {
        ...
        if (!directory) {
            ...
            directory = (0, workdir_js_1.dotGenaiscriptPath)("git", ...url.pathname.split(/\//g).filter((s) => !!s), branch || `HEAD`, sha);
        }
        (0, util_js_1.logVerbose)(`git: shallow cloning ${repository} to ${directory}`);
        ...
        if (install) {
            ...
            const { command, args: installArgs } = await (0, packagemanagers_js_1.packageResolveInstall)(directory);
            if (command) {
