...
exports.promptyParse = promptyParse;
exports.promptyToGenAIScript = promptyToGenAIScript;
const frontmatter_js_1 = require("./frontmatter.js");
const yaml_js_1 = require("./yaml.js");
const cleaners_js_1 = require("./cleaners.js");
const json5_js_1 = require("./json5.js");
function promptyFrontmatterToMeta(frontmatter) {
    const { name, description, tags = [], sample, inputs, outputs, model, files, tests, } = frontmatter;
    ...
    if (api !== "chat")
        throw new Error("completion api not supported");
    if (modelParameters?.n > 1)
        throw new Error("multi-turn not supported");
    ...
    const meta = (0, cleaners_js_1.deleteUndefinedValues)({
        model: modelName,
        title: name,
        description,
        files,
 ...
 */
function promptyParse(filename, text) {
 ...
 */
function promptyToGenAIScript(doc) {
    const { messages, meta } = doc;
    const renderJinja = (content) => `$\`${content.replace(/`/g, "\\`")}\`${/\{(%|\{)/.test(content) ? `.jinja(env.vars)` : ""}`;
    const renderPart = (c) => c.type === "text"
        ? renderJinja(c.text)
        : c.type === "image_url"
    ...
    if (Object.keys(meta).length) {
        src += `script(${(0, json5_js_1.JSON5Stringify)(meta, null, 2)})\n\n`;
    }
    src += messages
        .map((msg) => {
        const { role, content } = msg;
        if (role === "assistant") {
            return `assistant(parsers.jinja(${JSON.stringify(content)}, env.vars))`;
        }
        else if (role === "system") {
            return `writeText(${JSON.stringify(content)}, { role: "system" })`;
        }
        else {
...
//# sourceMappingURL=prompty.js.map