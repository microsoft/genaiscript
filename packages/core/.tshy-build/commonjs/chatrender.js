...
exports.renderShellOutput = renderShellOutput;
exports.renderMessageContent = renderMessageContent;
exports.lastAssistantReasoning = lastAssistantReasoning;
exports.renderMessagesToMarkdown = renderMessagesToMarkdown;
exports.collapseChatMessages = collapseChatMessages;
 ...
 */
function renderShellOutput(output) {
    ...
    return ([
        ...
        stdout ? `STDOUT:${(0, mkmd_js_1.fenceMD)(stdout, "text")}` : undefined,
        // Include stderr if it exists, formatted as text.
        stderr ? `STDERR:${(0, mkmd_js_1.fenceMD)(stderr, "text")}` : undefined,
    ]
 ...
 */
async function renderMessageContent(msg, options) {
    ...
    if (typeof content === "string") {
        if (textLang === "raw")
            ...
            return (0, mkmd_js_1.fenceMD)(content, textLang);
    }
            ...
            switch (c.type) {
                case "text":
                    if (textLang === "raw")
                        ...
                        res.push((0, mkmd_js_1.fenceMD)(c.text, textLang));
                    break;
                case "image_url":
 ...
 */
function lastAssistantReasoning(messages) {
 ...
 */
async function renderMessagesToMarkdown(messages, options) {
    // Set default options for filtering message roles.
    const { textLang = "markdown", system = undefined, // Include system messages unless explicitly set to false.
    user = undefined, // Include user messages unless explicitly set to false.
    assistant = true, // Include assistant messages by default.
    cancellationToken, tools, } = options || {};
    options = {
        textLang,
        system,
        user,
    ...
    const res = [];
    if (tools?.length) {
        res.push((0, mkmd_js_1.details)(`🔧 tools (${tools.length})`, tools
    ...
    for (const msg of messages?.filter((msg) => {
        ...
        switch (role) {
            case "system":
                res.push((0, mkmd_js_1.details)("📙 system", await renderMessageContent(msg, optionsMarkdown), false));
                break;
            case "user":
                res.push((0, mkmd_js_1.details)(`👤 user`, await renderMessageContent(msg, options), user === true));
                break;
            case "assistant":
                res.push((0, mkmd_js_1.details)(`🤖 assistant ${msg.name ? msg.name : ""}`, [
                    msg.reasoning_content
                        ? (0, mkmd_js_1.details)("🤔 reasoning", (0, mkmd_js_1.fenceMD)(msg.reasoning_content, "markdown"))
                        : undefined,
                    await renderMessageContent(msg, optionsMarkdown),
                    ...(msg.tool_calls?.map((tc) => (0, mkmd_js_1.details)(`📠 tool call <code>${tc.function.name}</code> (<code>${tc.id}</code>)`, renderToolArguments(tc.function.arguments))) || []),
                ]
                    .filter((s) => !!s)
                    .join("\n\n"), assistant === true));
                break;
            case "tool":
                res.push((0, mkmd_js_1.details)(`🛠️ tool output <code>${msg.tool_call_id}</code>`, await renderMessageContent(msg, {
                    ...(options || {}),
                    textLang: "json",
                })));
                break;
            default:
                res.push(role, (0, mkmd_js_1.fenceMD)(JSON.stringify(msg, null, 2), "json"));
                break;
        }
    }
    // Join the result array into a single markdown string.
    return (0, cleaners_js_1.collapseNewlines)(res.filter((s) => s !== undefined).join("\n"));
}
/**
...
function renderToolArguments(args) {
    const js = (0, json5_js_1.JSONLLMTryParse)(args);
    // Convert arguments to YAML if possible, otherwise keep as JSON.
    if (js)
        return (0, mkmd_js_1.fenceMD)((0, yaml_1.stringify)(js), "yaml");
    else
        return (0, mkmd_js_1.fenceMD)(args, "json");
}
/**
 ...
 */
function assistantText(messages, options) {
    ...
    text = (0, think_js_1.unthink)(text);
    if ((!responseType && !responseSchema) || responseType === "markdown") {
        text = (0, unwrappers_js_1.unfence)(text, ["markdown", "md"]);
    }
    else if (responseType === "yaml") {
        text = (0, unwrappers_js_1.unfence)(text, ["yaml", "yml"]);
    }
    else if (/^json/.test(responseType)) {
        text = (0, unwrappers_js_1.unfence)(text, ["json", "json5"]);
    }
    else if (responseType === "text") {
        text = (0, unwrappers_js_1.unfence)(text, ["text", "txt"]);
    }
    return text;
}
