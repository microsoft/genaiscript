...
exports.diagnosticsToCSV = diagnosticsToCSV;
 ...
 */
function diagnosticsToCSV(diagnostics, sep) {
    return diagnostics
 ...
 */
function collectFolders(prj, options) {
    const { force } = options || {};
    const { systemDir } = prj;
    const folders = {};
    ...
    (script) => script.filename)) {
        const dir = (0, node_path_1.dirname)(t.filename); // Get directory name from the filename
        if (!force && (0, node_path_1.resolve)(dir) === systemDir)
            continue;
        const folder = folders[dir] || (folders[dir] = { dirname: dir });
        folder.js = folder.js || constants_js_1.GENAI_ANYJS_REGEX.test(t.filename); // Check for presence of JS files
        folder.ts = folder.ts || constants_js_1.GENAI_ANYTS_REGEX.test(t.filename); // Check for presence of TS files
    }
 ...
 */
function resolveScript(prj, system) {
...
function filterScripts(scripts, options) {
    ...
    return scripts
        .filter((t) => !test || (0, cleaners_js_1.arrayify)(t.tests)?.length)
        .filter((t) => !redteam || t.redteam)
        .filter((t) => !ids?.length || ids.includes(t.id))
        .filter((t) => unlisted || !t.unlisted)
        .filter((t) => (0, tags_js_1.tagFilter)(groups, t.group));
}
