...
const dbg = (0, debug_js_1.genaiscriptDebug)("splitter");
/**
...
class TextSplitter {
    ...
    split(text) {
        ...
        (0, console_1.assert)(!chunks.some((c) => !c.text), `TextSplitter returned empty chunk.`);
        const getOverlapTokens = (tokens) => {
            if (tokens != undefined) {
                const len = tokens.length > this._config.chunkOverlap ? this._config.chunkOverlap : tokens.length;
    ...
    recursiveSplit(text, separators, startPos) {
        ...
        if (text.length > 0) {
            ...
            for (let i = 0; i < parts.length; i++) {
                ...
                if (!this._config.keepSeparators && !this.containsAlphanumeric(chunk)) {
                    continue;
                }
