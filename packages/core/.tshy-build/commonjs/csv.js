...
exports.CSVTryParse = CSVTryParse;
exports.CSVStringify = CSVStringify;
exports.dataToMarkdownTable = dataToMarkdownTable;
exports.headersToMarkdownTableSeparator = headersToMarkdownTableSeparator;
exports.headersToMarkdownTableHead = headersToMarkdownTableHead;
exports.objectToMarkdownTableRow = objectToMarkdownTableRow;
exports.CSVChunk = CSVChunk;
 ...
 */
function CSVParse(text, options) {
    ...
    const columns = headers ? (0, cleaners_js_1.arrayify)(headers) : true;
 ...
 */
function CSVTryParse(text, options) {
    const { trace } = options || {};
    try {
        if (!text)
 ...
 */
function dataToMarkdownTable(csv, options) {
    if (!csv?.length)
        return ""; // Return empty string if CSV is empty
    const headers = (0, cleaners_js_1.arrayify)(options?.headers);
    if (headers.length === 0)
        headers.push(...Object.keys(csv[0])); // Use object keys as headers if not provided
    const res = [
 ...
 */
function headersToMarkdownTableSeparator(headers) {
 ...
 */
function headersToMarkdownTableHead(headers) {
    return `|${headers.join("|")}|\n`;
}
/**
 ...
 */
function objectToMarkdownTableRow(row, headers, options) {
 ...
 */
function CSVChunk(rows, size) {
    return (0, es_toolkit_1.chunk)(rows || [], Math.max(1, size | 0)).map((rows, chunkStartIndex) => ({
        chunkStartIndex,
        rows,
    }));
}
