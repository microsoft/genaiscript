...
exports.indexToLineNumber = indexToLineNumber;
...
function addLineNumbers(text, options) {
    ...
    if (language === "diff" || (0, diff_js_1.tryDiffParse)(text)) {
        const diffed = (0, llmdiff_js_1.llmifyDiff)(text); // Process the text with a special function for diffs
        if (diffed !== undefined)
            return diffed; // Return processed text if diff handling was successful
    }
    // don't add line numbers for small files
    const lines = text.split("\n"); // Split text into lines
    if (startLine === 1 && lines.length < constants_js_1.MIN_LINE_NUMBER_LENGTH)
        return text;
    return lines
...
function removeLineNumbers(text) {
    ...
    if (!lines.slice(0, 10).every((line) => rx.test(line)))
        return text; // Return original text if not all lines have numbers
 ...
 */
function extractRange(text, options) {
...
function indexToLineNumber(text, index) {
    if (text === undefined || text === null || index < 0 || index >= text.length)
        return -1;
    let lineNumber = 1;
