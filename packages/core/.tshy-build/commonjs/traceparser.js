...
exports.parseTraceTree = parseTraceTree;
exports.renderTraceTree = renderTraceTree;
 ...
 */
function parseTraceTree(text, options) {
    ...
    for (let i = 0; i < lines.length; ++i) {
        ...
        if (startDetails) {
            ...
            if (openeds) {
                if (openeds.has(current.id))
                    current.open = true;
            }
            else if (startDetails[1].includes("open"))
                current.open = true;
 ...
 */
function renderTraceTree(node, level) {
    if (!node)
        return "";
    if (typeof node === "string") {
        ...
        return (0, mkmd_js_1.details)(node.label, level > 0 ? node.content.map((n) => renderTraceTree(n, level - 1)).join("\n") : "...");
    else
        return "";
}
//# sourceMappingURL=traceparser.js.map