...
exports.srtVttRender = srtVttRender;
exports.parseTimestamps = parseTimestamps;
exports.vttSrtParse = vttSrtParse;
const srt_vtt_parser_1 = require("@plussub/srt-vtt-parser");
 ...
 */
function srtVttRender(transcription) {
    ...
    function formatVRTTime(seconds) {
        const date = new Date(0);
        date.setMilliseconds(seconds * 1000);
        const hours = String(date.getUTCHours()).padStart(2, "0");
        const minutes = String(date.getUTCMinutes()).padStart(2, "0");
 ...
 */
function parseTimestamps(transcription) {
    const ts = [];
 ...
 */
function vttSrtParse(transcription) {
    if (!transcription)
        return [];
    const p = (0, srt_vtt_parser_1.parse)(transcription);
    return p.entries.map((e) => (0, cleaners_js_1.deleteEmptyValues)({
        id: e.id,
        start: e.from,
        end: e.to,
        text: e.text,
