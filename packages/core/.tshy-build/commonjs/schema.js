...
exports.JSONSchemaToFunctionParameters = JSONSchemaToFunctionParameters;
exports.JSONSchemaStringifyToTypeScript = JSONSchemaStringifyToTypeScript;
exports.validateSchema = validateSchema;
exports.tryValidateJSONWithSchema = tryValidateJSONWithSchema;
exports.validateJSONWithSchema = validateJSONWithSchema;
exports.validateFencesWithSchema = validateFencesWithSchema;
exports.JSONSchemaStringify = JSONSchemaStringify;
exports.toStrictJSONSchema = toStrictJSONSchema;
exports.JSONSchemaInfer = JSONSchemaInfer;
...
const dbg = (0, debug_js_1.genaiscriptDebug)("schema");
/**
 ...
 */
function JSONSchemaToFunctionParameters(schema) {
    return renderJSONSchemaToFunctionParameters(schema, 0);
}
function renderJSONSchemaToFunctionParameters(schema, depth) {
    depth = depth + 1;
    if (!schema)
        return "";
        ...
        if (single.type === "array") {
            ...
            return `${depth > 1 ? `{ ` : ""}${Object.entries(single.properties)
                .sort((l, r) => (required.includes(l[0]) ? -1 : 1) - (required.includes(r[0]) ? -1 : 1))
                .map(([name, prop]) => `${name}${required.includes(name) ? "" : "?"}: ${renderJSONSchemaToFunctionParameters(prop, depth)}`)
                .join(", ")}${depth > 1 ? " }" : ""}`;
        }
        else if (single.type === "string")
            return "string";
 ...
 */
function JSONSchemaStringifyToTypeScript(schema, options) {
    const { typeName = "Response" } = options || {};
    const lines = []; // Array to accumulate lines of TypeScript code
    ...
    append(`${options?.export ? "export " : ""}type ${typeName.replace(/\s+/g, "_")} =`);
    ...
    function stringifyNodeDoc(node) {
        const n = node;
        ...
        switch (n.type) {
            ...
            case "integer": {
                ...
                if (n.maximum !== undefined)
                    doc.push(`maximum: ${n.maximum}`);
                break;
            }
            case "string": {
                if (n.pattern)
                    doc.push(`pattern: ${n.pattern}`);
                break;
            }
        }
...
function tryValidateJSONWithSchema(object, options) {
...
object, schema, options) {
    ...
    try {
        ...
        dbg(`runtime error: ${(0, error_js_1.errorMessage)(e)}`);
        trace?.warn("schema validation failed");
        return { schema, pathValid: false, schemaError: (0, error_js_1.errorMessage)(e) };
    }
}
/**
 ...
 */
function validateFencesWithSchema(fences, schemas, options) {
    const frames = [];
    // Validate schemas in fences
    for (const fence of fences?.filter(({ language, args }) => args?.schema && (language === "json" || language === "yaml"))) {
        ...
        try {
            if (language === "json")
                data = (0, json5_js_1.JSON5parse)(val, { repair: true });
            else if (language === "yaml")
                data = (0, yaml_js_1.YAMLParse)(val);
        }
        catch (e) {
            fence.validation = {
                pathValid: false,
        ...
        if (!fence.validation) {
            ...
            if (!schemaObj) {
                ...
                fence.validation = validateJSONWithSchema(data, schemaObj, options);
        }
        // Add to frames
        frames.push({
            schema,
 ...
 */
function JSONSchemaStringify(schema) {
 ...
 */
function toStrictJSONSchema(schema, options) {
    const { noDefaults, defaultOptional } = options || {};
    const clone = structuredClone((0, parameters_js_1.promptParametersSchemaToJSONSchema)(schema, { noDefaults }));
    visit(clone);
        ...
        switch (n.type) {
            ...
            case "object": {
                ...
                for (const key in n.properties) {
                    ...
                    const child = n.properties[key];
                    visit(child);
                    if (!defaultOptional && !n.required.includes(key)) {
                        n.required.push(key);
                        if (["string", "number", "boolean", "integer"].includes(child.type)) {
 ...
 */
async function JSONSchemaInfer(obj) {
    const res = (0, parameters_js_1.promptParametersSchemaToJSONSchema)(obj, { noDefaults: true });
    return res;
}
