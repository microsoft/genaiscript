...
exports.extractFenced = extractFenced;
exports.findFirstDataFence = findFirstDataFence;
exports.parseVars = parseVars;
exports.renderFencedVariables = renderFencedVariables;
 ...
 */
function startFence(text) {
    const m = promptFenceStartRx.exec(text);
    const groups = m?.groups || {};
    return {
        fence: groups.fence,
        language: (0, unwrappers_js_1.unquote)(groups.language),
 ...
 */
function parseKeyValuePair(text) {
    const m = /[=:]/.exec(text);
    return m ? { [text.slice(0, m.index)]: (0, unwrappers_js_1.unquote)(text.slice(m.index + 1)) } : {};
}
/**
...
function parseKeyValuePairs(text) {
    const res = {};
    const chunks = (0, cleaners_js_1.arrayify)(text);
    chunks.forEach((chunk) => chunk
        ?.split(/\s+/g)
        .map((kv) => kv.split(/[=:]/))
        .filter((m) => m.length == 2)
        .forEach((m) => (res[m[0]] = (0, unwrappers_js_1.unquote)(m[1]))));
...
function extractFenced(text) {
    ...
    let currArgs = {}; // Arguments parsed from the fence
    let currFence = ""; // Current fence delimiter
    const vars = []; // Array to store the fenced blocks
        ...
        if (currFence) {
            ...
            if (fence.fence && fence.args["file"]) {
                ...
                if (start.fence && line.endsWith(":")) {
                    currLbl = ((0, unwrappers_js_1.unquote)(line.slice(0, -1)) + " " + (start.args["file"] || "")).trim();
                    ...
                    currLbl = (0, unwrappers_js_1.unquote)(m[1]) + " " + (start.args["file"] || (0, unwrappers_js_1.unquote)(m[2]));
                    currFence = start.fence;
                    currLanguage = start.language || "";
                    currArgs = start.args;
                    i++;
     ...
     */
    function normalize(label, text) {
        // remove extra line numbers
        text = (0, liner_js_1.removeLineNumbers)(text);
 ...
 */
function findFirstDataFence(fences) {
    const { content, language } = fences?.find((f) => f.content && !f.label && (f.language === "yaml" || f.language === "json")) || {};
    if (language === "yaml" || language === "yml")
        return (0, yaml_js_1.YAMLTryParse)(content);
    else if (language === "json")
        return (0, json5_js_1.JSON5TryParse)(content);
    return undefined;
}
/**
...
function parseVars(vars) {
    ...
    if (vars)
        for (const v of vars)
            Object.assign(res, parseKeyValuePairs(v));
    return Object.freeze(res);
}
/**
...
function renderFencedVariables(vars) {
    return vars
        .map(({ label: k, content: v, validation, args, language }) => `-   ${k ? `\`${k}\`` : ""} ${validation !== undefined
        ? `${validation.schemaError ? constants_js_1.EMOJI_UNDEFINED : validation.pathValid === false ? constants_js_1.EMOJI_FAIL : constants_js_1.EMOJI_SUCCESS}`
        : "no label"}\n
