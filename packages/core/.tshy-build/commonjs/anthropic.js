...
exports.AnthropicBedrockModel = exports.AnthropicModel = void 0;
...
const dbg = (0, debug_1.default)("genaiscript:anthropic");
const dbgMessages = (0, debug_1.default)("genaiscript:anthropic:msg");
const convertFinishReason = (stopReason) => {
    switch (stopReason) {
        case "end_turn":
            return "stop";
        case "max_tokens":
...
const adjustUsage = (usage, outputTokens) => {
    return {
        ...usage,
...
const convertMessages = (messages, emitThinking) => {
    const res = [];
...
const convertSingleMessage = (msg, emitThinking) => {
    const { role } = msg;
    if (!role) {
        return {
            role: "user",
        ...
        throw new error_js_1.NotSupportedError("function message not supported");
    return convertStandardMessage(msg);
};
function toCacheControl(msg) {
    return msg.cacheControl === "ephemeral" ? { type: "ephemeral" } : undefined;
}
const convertAssistantMessage = (msg, emitThinking) => {
    return {
        role: "assistant",
        content: [
            ...
            ...(convertStandardMessage(msg)?.content || []),
            ...(msg.tool_calls || []).map((tool) => (0, cleaners_js_1.deleteUndefinedValues)({
                type: "tool_use",
                id: tool.id,
                input: (0, json5_js_1.JSONLLMTryParse)(tool.function.arguments),
...
const convertToolResultMessage = (msg) => {
    return {
        role: "user",
        content: [
...
const convertBlockParam = (block, cache_control) => {
...
const convertStandardMessage = (msg) => {
    ...
    if (Array.isArray(msg.content)) {
        ...
        res = {
            ...
            content: msg.content
                ...
                .filter((t) => !!t)
                .map(cleaners_js_1.deleteUndefinedValues),
        };
    }
...
const convertImageUrlBlock = (block) => {
    return {
        ...
        source: {
            type: "base64",
            media_type: block.image_url.url.startsWith("data:image/png") ? "image/png" : "image/jpeg",
...
const completerFactory = (resolver) => {
    const completion = async (req, cfg, options, trace) => {
        const { requestOptions, partialCb, cancellationToken, inner, retries, maxDelay, maxRetryAfter, retryDelay, } = options;
        const { headers } = requestOptions || {};
        const { provider, model, reasoningEffort } = (0, models_js_1.parseModelIdentifier)(req.model);
        ...
        const caching = /sonnet|haiku|opus/i.test(model) && req.messages.some((m) => m.cacheControl === "ephemeral");
        const httpAgent = await (0, proxy_js_1.resolveUndiciProxyAgent)();
        ...
        const tool_choice = req.tool_choice === "auto"
            ...
            : req.tool_choice === "none"
                ? { type: "none" }
                : req.tool_choice !== "required" && typeof req.tool_choice === "object"
                    ? {
                        type: "tool",
        ...
        const reasoningEfforts = (0, features_js_1.providerFeatures)(provider)?.reasoningEfforts;
        const budget_tokens = reasoningEfforts[req.reasoning_effort || reasoningEffort];
        let max_tokens = req.max_tokens;
        if (budget_tokens && (!max_tokens || max_tokens < budget_tokens))
            max_tokens = budget_tokens + constants_js_1.ANTHROPIC_MAX_TOKEN;
        max_tokens = max_tokens || constants_js_1.ANTHROPIC_MAX_TOKEN;
        if (budget_tokens) {
        ...
        const mreq = (0, cleaners_js_1.deleteUndefinedValues)({
            model,
            tools,
            messages,
            max_tokens,
        ...
        try {
            ...
            for await (const chunk of stream) {
                ...
                switch (chunk.type) {
                        ...
                        switch (chunk.delta.type) {
                            ...
                            case "text_delta":
                                if (!chunk.delta.text)
                                    ...
                                    numTokens += (0, tokens_js_1.approximateTokens)(chunkContent, { encoder });
                                    chatResp += chunkContent;
                                    trace?.appendToken(chunkContent);
                                }
                                break;
                            case "input_json_delta":
                                toolCalls[chunk.index].arguments += chunk.delta.partial_json;
                        }
                        break;
                    ...
                    case "message_delta":
                        if (chunk.delta.stop_reason) {
                            finishReason = convertFinishReason(chunk.delta.stop_reason);
                        }
                        if (chunk.usage) {
                ...
                if (chunkContent || reasoningContent) {
                    const progress = (0, cleaners_js_1.deleteUndefinedValues)({
            ...
            trace?.error("error while processing event", (0, error_js_1.serializeError)(e));
        }
        trace?.appendContent("\n\n");
...
exports.AnthropicModel = Object.freeze({
    ...
    id: constants_js_1.MODEL_PROVIDER_ANTHROPIC,
    listModels,
});
exports.AnthropicBedrockModel = Object.freeze({
    completer: completerFactory(async (trace, cfg, httpAgent, fetch) => {
        const AnthropicBedrockClass = (await import("@anthropic-ai/bedrock-sdk")).default;
        const anthropic = new AnthropicBedrockClass({
            baseURL: cfg.base,
            fetch,
            fetchOptions: {
                dispatcher: httpAgent,
    ...
    id: constants_js_1.MODEL_PROVIDER_ANTHROPIC_BEDROCK,
    listModels: async () => {
        return {
            ok: true,
            models: [
