...
exports.azureDevOpsParseEnv = azureDevOpsParseEnv;
exports.azureDevOpsUpdatePullRequestDescription = azureDevOpsUpdatePullRequestDescription;
exports.azureDevOpsCreateIssueComment = azureDevOpsCreateIssueComment;
const fetch_js_1 = require("./fetch.js");
const githubclient_js_1 = require("./githubclient.js");
const pretty_js_1 = require("./pretty.js");
const util_js_1 = require("./util.js");
const debug_js_1 = require("./debug.js");
const dbg = (0, debug_js_1.genaiscriptDebug)("azuredevops");
/**
 ...
 */
async function azureDevOpsParseEnv(env) {
...
async function findPullRequest(info) {
    ...
    const fetch = await (0, fetch_js_1.createFetch)({ retryOn: [] });
    ...
    if (resGet.status !== 200) {
        (0, util_js_1.logError)(`pull request search failed, ${resGet.status}: ${resGet.statusText}`);
        return undefined;
    }
    const resGetJson = (await resGet.json());
    const pr = resGetJson?.value?.[0];
    if (!pr) {
        (0, util_js_1.logError)(`pull request not found`);
        return undefined;
    }
    return pr;
}
 ...
 */
async function azureDevOpsUpdatePullRequestDescription(script, info, text, commentTag) {
    ...
    text += (0, githubclient_js_1.generatedByFooter)(script, info);
    description = (0, githubclient_js_1.mergeDescription)(commentTag, description, text);
    ...
    if (res.status !== 200)
        (0, util_js_1.logError)(`pull request update failed, ${res.status}: ${res.statusText}`);
    else
        (0, util_js_1.logVerbose)(`pull request updated`);
}
/**
 ...
 */
async function azureDevOpsCreateIssueComment(script, info, body, commentTag) {
    ...
    if (isNaN(pullRequestId))
        return;
    const fetch = await (0, fetch_js_1.createFetch)({ retryOn: [] });
    body += (0, githubclient_js_1.generatedByFooter)(script, info);
    const Authorization = `Bearer ${accessToken}`;
    ...
    if (commentTag) {
        ...
        const openThreads = threads.value?.filter((c) => c.status === "active" && c.comments?.some((comment) => comment.content.includes(tag))) || [];
        for (const thread of openThreads) {
            (0, util_js_1.logVerbose)(`pull request closing old comment thread ${thread.id}`);
    ...
    if (res.status !== 200) {
        (0, util_js_1.logError)(`pull request comment creation failed, ${res.statusText}`);
        dbg(await (0, fetch_js_1.tryReadText)(res));
    }
    else
        (0, util_js_1.logVerbose)(`pull request comment created`);
}
