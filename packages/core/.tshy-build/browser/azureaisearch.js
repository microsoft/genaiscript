...
import { checkCancelled, toSignal } from "./cancellation.js";
...
export const azureAISearchIndex = async (indexName, cfg, embedder, options) => {
    ...
    logVerbose(`azure ai search: ${indexName}, embedder ${cfg.provider}:${cfg.model}, ${vectorSize} dimensions`);
    ...
    const chunkId = async (textChunk) => await hash([textChunk.filename ?? textChunk.content, textChunk.lineEnd, textChunk.lineEnd], {
        length: HASH_LENGTH,
    });
    return Object.freeze({
        ...
        insertOrUpdate: async (file) => {
            ...
            logVerbose(`azure ai search: ${indexName} index ${outdated.length} outdated, ${docs.length} updated`);
            if (outdated.length) {
