 ...
 */
export async function fileWriteCached(dir, bufferLike, options) {
    const { bytes, ext: sourceExt } = await resolveBufferLikeAndExt(bufferLike, options);
    if (!bytes) {
        // file empty
        return undefined;
    }
...
export async function fileWriteCachedJSON(dir, data) {
 ...
 */
export async function fileCacheImage(url, options) {
    if (!url)
        return "";
    const filename = filenameOrFileToFilename(url);
    if (typeof filename === "string" && HTTPS_REGEX.test(filename))
        return filename;
    const { dir = dotGenaiscriptPath("images"), trace, cancellationToken } = options || {};
...
export function patchCachedImages(text, patcher) {
