 ...
 */
export function fenceMD(t, contentType) {
    if (t === undefined)
        return undefined;
    contentType = contentTypes[contentType] || contentType || "";
    let f = "```";
    while (t.includes(f) && f.length < 8)
        f += "`"; // Extend fence if necessary
    return `\n${f}${contentType}\n${trimNewlines(t)}\n${f}\n`;
}
/**
 ...
 */
export function details(summary, body, open) {
    return `\n<details${open ? " open" : ""}>
