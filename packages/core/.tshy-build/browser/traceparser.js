 ...
 */
export function parseTraceTree(text, options) {
 ...
 */
export function renderTraceTree(node, level) {
    if (!node)
        return "";
    if (typeof node === "string") {
        ...
        return details(node.label, level > 0 ? node.content.map((n) => renderTraceTree(n, level - 1)).join("\n") : "...");
    else
        return "";
}
