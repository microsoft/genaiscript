...
function tryReadFile(fn) {
    return host.readFile(fn).then((r) => r, (_) => null);
}
/**
 ...
 */
export function isJSONLFilename(fn) {
 ...
 */
export function JSONLTryParse(text, options) {
    if (!text)
        return [];
    const res = [];
 ...
 */
export function JSONLStringify(objs) {
    if (!objs?.length)
        return "";
    const acc = [];
 ...
 */
export async function writeJSONL(fn, objs) {
 ...
 */
export async function appendJSONL(name, objs, meta) {
