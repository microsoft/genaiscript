 ...
 */
export async function computeFileEdits(res, options) {
            ...
            try {
                ...
                for (const changelog of cls) {
                    ...
                    const fn = /^[^\/]/.test(filename) // TODO
                        ...
                        : filename;
                    const fileEdit = await getFileEdit(fn);
                    fileEdit.after = applyChangeLog(fileEdit.after || fileEdit.before || "", changelog);
                }
            }
            catch (e) {
                logError(e);
    ...
    if (edits.length)
        trace?.details("✏️ edits", dataToMarkdownTable(edits, {
                            ...
                            if (/\.(json|yaml)$/i.test(fileEditName)) {
                                ...
                                if (schemaId) {
                                    ...
                                    if (!schema)
                                        ...
                                        fe.validation = validateJSONWithSchema(data, schema, {
                                            trace,
                                        });
                                }
                            }
 ...
 */
export async function writeFileEdits(fileEdits, // Contains the edits to be applied to files
options) {
    ...
    for (const fileEdit of Object.entries(fileEdits || {})) {
        ...
        if (after !== before) {
            ...
            trace?.detailsFenced(`updating ${fn}`, diffCreatePatch({ filename: fn, content: before }, { filename: fn, content: after }), "diff");
