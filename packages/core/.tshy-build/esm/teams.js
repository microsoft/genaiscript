...
async function microsoftTeamsChannelUploadFile(token, channelUrl, file, options) {
    ...
    const itemUrl = `https://graph.microsoft.com/v1.0/groups/${teamId}/drive/root:/${root}/${folder}/${basename(filename)}`;
 ...
 */
export async function microsoftTeamsChannelPostMessage(channelUrl, message, options) {
...
class MicrosoftTeamsChannelClient {
    ...
    async postMessage(message, options) {
        ...
        const res = await microsoftTeamsChannelPostMessage(this.channelUrl, dedent(message), {
            files,
            disclaimer,
        });
        return res.webUrl;
 ...
 */
export function createMicrosoftTeamsChannelClient(url) {
    if (!url)
