...
import { GITHUB_API_VERSION, GITHUB_ASSET_BRANCH, GITHUB_ASSET_URL_RX, GITHUB_PULL_REQUEST_REVIEW_COMMENT_LINE_DISTANCE, GITHUB_REST_API_CONCURRENCY_LIMIT, GITHUB_REST_PAGE_DEFAULT, GITHUB_TOKENS, TOOL_ID, } from "./constants.js";
...
async function githubFromEnv(env) {
    ...
    let issue = normalizeInt(env.GITHUB_ISSUE ??
 ...
 */
export async function githubUpdatePullRequestDescription(script, info, text, commentTag, options) {
 ...
 */
export function mergeDescription(commentTag, body, text) {
    body = body ?? "";
 ...
 */
export function generatedByFooter(script, info, code, stats) {
 ...
 */
export function appendGeneratedComment(script, info, annotation) {
...
export async function githubCreateIssueComment(script, info, body, commentTag, options) {
...
async function githubCreatePullRequestReview(script, info, token, annotation, existingComments, options) {
    ...
    if (existingComments.find((c) => c.path === body.path &&
        Math.abs(c.line - body.line) < GITHUB_PULL_REQUEST_REVIEW_COMMENT_LINE_DISTANCE &&
        (annotation.code ? c.body?.includes(annotation.code) : c.body?.includes(prettyMessage)))) {
        logVerbose(`pull request ${commitSha} comment creation already exists, skipping`);
    ...
    if (!r.created) {
        logVerbose(`pull request ${commitSha} comment creation failed, ${r.statusText} (${res.status})`);
 ...
 */
export async function githubCreatePullRequestReviews(script, info, annotations, options) {
    const { cancellationToken } = options ?? {};
    ...
    if (failed.length) {
        await githubCreateIssueComment(script, info, failed.map((d) => diagnosticToGitHubMarkdown(info, d)).join("\n\n"), script.id + "-prr", options);
    }
    return true;
}
...
export class GitHubClient {
    ...
    async listGists(options) {
        ...
        const { count = GITHUB_REST_PAGE_DEFAULT, filenameAsResources, ...rest } = options ?? {};
    ...
    async getGist(gist_id) {
        ...
        const res = {
            ...
            description,
            created_at,
            files: Object.values(files).map(({ filename, content, size, encoding }) => deleteUndefinedValues({
                filename,
                content,
    ...
    async assignIssueToBot(issue_number, options) {
        ...
        // assign
        const updated = await this.graphql(dedent `mutation {
  replaceActorsForAssignable(input: {assignableId: "${issue.node_id}" actorIds: ["${actor.id}"]}) {
    ...
    async getRepositoryContent(path, options) {
        ...
        if (downloadContent) {
            const limit = concurrentLimit("github", GITHUB_REST_API_CONCURRENCY_LIMIT);
            await Promise.all(res
                ...
                return async () => {
                    ...
                    f.content = Buffer.from(arrayify(fileContent)[0].content, "base64").toString("utf8");
                };
            })
                .map((p) => limit(p)));
        }
...
export function cleanLog(text) {
    return shellRemoveAsciiColors(text.replace(
    // timestamps
