...
import { DOT_ENV_FILENAME, DOT_ENV_GENAISCRIPT_FILENAME, MODEL_PROVIDERS, TOOL_ID, } from "./constants.js";
...
export function mergeHostConfigs(config, parsed) {
    if (!config && !parsed)
        return undefined;
    if (!parsed)
        return config;
...
async function resolveGlobalConfiguration(dotEnvPaths, hostConfig) {
    ...
    for (const dir of dirs) {
        for (const ext of exts) {
            ...
            const parsed = ext === "yml" || ext === "yaml" ? YAMLTryParse(fileContent) : JSON5TryParse(fileContent);
            if (!parsed) {
                dbg(`failed to parse ${filename}`);
                throw new Error(`config: failed to parse ${filename}`);
            }
            dbg("validating config schema");
            const validation = validateJSONWithSchema(parsed, schema);
 ...
 */
export async function readHostConfig(dotEnvPaths, hostConfig) {
 ...
 */
export async function resolveLanguageModelConfigurations(provider, options) {
    const { token, error, models, hide } = options || {};
    const res = [];
    dbg("starting to resolve language model configurations");
    for (const modelProvider of MODEL_PROVIDERS.filter((mp) => (!provider || mp.id === provider) && (!hide || !mp.hidden))) {
        dbg(`processing model provider: ${modelProvider.id}, token: ${token}`);
        try {
            const conn = await host.getLanguageModelConfiguration(modelProvider.id + ":*", options);
            if (conn) {
