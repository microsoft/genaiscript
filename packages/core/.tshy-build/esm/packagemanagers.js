 ...
 */
export async function packageResolveInstall(cwd) {
...
export async function packageResolveExecute(cwd, args, options) {
    dbg(`resolving`);
    ...
    if (agent === "npm")
        args.unshift("--yes");
    const resolved = resolveCommand(agent, "execute", args.filter((a) => a !== undefined));
