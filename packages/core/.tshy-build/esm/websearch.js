...
import { BING_SEARCH_ENDPOINT, DOCS_WEB_SEARCH_BING_SEARCH_URL, DOCS_WEB_SEARCH_TAVILY_URL, TAVILY_ENDPOINT, } from "./constants.js";
 ...
 */
export async function bingSearch(q, options) {
    const { ignoreMissingApiKey, trace, endPoint = BING_SEARCH_ENDPOINT, count, cc, freshness, responseFilter = "Webpages", safeSearch = "strict", } = options || {};
    ...
    if (!apiKey) {
        ...
        throw new Error(`BING_SEARCH_API_KEY secret is required to use bing search. See ${DOCS_WEB_SEARCH_BING_SEARCH_URL}.`, { cause: "missing key" });
    }
    try {
        ...
        return (json.webPages?.value?.map(({ snippet, url }) => ({
            filename: url,
            content: snippet,
        })) || []);
    }
 ...
 */
export async function tavilySearch(q, options) {
    ...
    if (!apiKey) {
        ...
        if (ignoreMissingApiKey)
            return undefined;
        throw new Error(`TAVILY_API_KEY secret is required to use Tavily search. See ${DOCS_WEB_SEARCH_TAVILY_URL}.`, { cause: "missing key" });
    }
    try {
