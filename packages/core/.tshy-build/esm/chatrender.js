 ...
 */
export function renderShellOutput(output) {
 ...
 */
export async function renderMessageContent(msg, options) {
    ...
    if (typeof content === "string") {
        if (textLang === "raw")
            ...
            return fenceMD(content, textLang);
    }
            ...
            switch (c.type) {
                case "text":
                    if (textLang === "raw")
                        ...
                        res.push(fenceMD(c.text, textLang));
                    break;
                case "image_url":
 ...
 */
export async function renderMessagesToMarkdown(messages, options) {
    ...
    const res = [];
    if (tools?.length) {
        res.push(details(`🔧 tools (${tools.length})`, tools
    ...
    for (const msg of messages?.filter((msg) => {
        ...
        switch (role) {
            ...
            case "user":
                ...
                break;
            case "assistant":
                res.push(details(`🤖 assistant ${msg.name ? msg.name : ""}`, [
                    msg.reasoning_content
                        ? details("🤔 reasoning", fenceMD(msg.reasoning_content, "markdown"))
                        : undefined,
                    await renderMessageContent(msg, optionsMarkdown),
                    ...(msg.tool_calls?.map((tc) => details(`📠 tool call <code>${tc.function.name}</code> (<code>${tc.id}</code>)`, renderToolArguments(tc.function.arguments))) || []),
                ]
                    .filter((s) => !!s)
                    .join("\n\n"), assistant === true));
                break;
            case "tool":
                res.push(details(`🛠️ tool output <code>${msg.tool_call_id}</code>`, await renderMessageContent(msg, {
                    ...(options || {}),
                    textLang: "json",
                })));
                break;
...
function renderToolArguments(args) {
    ...
    if (js)
        return fenceMD(YAMLStringify(js), "yaml");
    else
        return fenceMD(args, "json");
}
/**
 ...
 */
export function assistantText(messages, options) {
