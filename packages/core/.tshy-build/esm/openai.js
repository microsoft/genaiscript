...
import { AZURE_AI_INFERENCE_VERSION, AZURE_OPENAI_API_VERSION, MODEL_PROVIDER_ALIBABA, MODEL_PROVIDER_AZURE_AI_INFERENCE, MODEL_PROVIDER_AZURE_OPENAI, MODEL_PROVIDER_AZURE_SERVERLESS_MODELS, MODEL_PROVIDER_AZURE_SERVERLESS_OPENAI, MODEL_PROVIDER_GITHUB, MODEL_PROVIDER_HUGGINGFACE, MODEL_PROVIDER_OPENAI, MODEL_PROVIDER_OPENAI_HOSTS, OPENROUTER_API_CHAT_URL, OPENROUTER_SITE_NAME_HEADER, OPENROUTER_SITE_URL_HEADER, THINK_END_TOKEN_REGEX, THINK_START_TOKEN_REGEX, TOOL_ID, TOOL_NAME, TOOL_URL, } from "./constants.js";
 ...
 */
export function getConfigHeaders(cfg) {
...
export const OpenAIChatCompletion = async (req, cfg, options, trace) => {
    ...
    let url = "";
    const toolCalls = [];
    if (cfg.type === MODEL_PROVIDER_OPENAI ||
        cfg.type === "localai" ||
        ...
        if (url === OPENROUTER_API_CHAT_URL) {
            headers[OPENROUTER_SITE_URL_HEADER] = process.env.OPENROUTER_SITE_URL || TOOL_URL;
            headers[OPENROUTER_SITE_NAME_HEADER] = process.env.OPENROUTER_SITE_NAME || TOOL_NAME;
        }
    }
        ...
        trace?.itemValue(`version`, version);
        url =
            trimTrailingSlash(cfg.base).replace(/^https?:\/\/(?<deployment>[^\.]+)\.(?<region>[^\.]+)\.models\.ai\.azure\.com/i, (m, deployment, region) => `https://${postReq.model}.${region}.models.ai.azure.com`) + `/chat/completions?api-version=${version}`;
        headers["extra-parameters"] = "pass-through";
        delete postReq.model;
        delete postReq.stream_options;
    }
    ...
    if (r.status !== 200) {
        ...
        const errors = JSON5TryParse(responseBody, {});
        const error = Array.isArray(errors) ? errors[0]?.error : errors;
        throw new RequestError(r.status, errorMessage(error) || r.statusText, errors, responseBody, normalizeInt(r.headers.get("retry-after")));
    }
    let done = false;
            ...
            if (typeof delta?.content === "string" && delta.content !== "") {
                ...
                if (!isEmptyString(content)) {
                    if (reasoning) {
                        ...
                        reasoningTokens.push(...serializeChunkChoiceToLogProbs(choice));
                    }
                    else {
                        numTokens += approximateTokens(content, { encoder });
                        chatResp += content;
                        tokens.push(...serializeChunkChoiceToLogProbs(choice));
                    }
...
export const OpenAIListModels = async (cfg, options) => {
    try {
 ...
 */
export async function OpenAITranscribe(req, cfg, options) {
    const { trace } = options || {};
    try {
        logVerbose(`${cfg.provider}: transcribe ${req.file.type} ${prettyBytes(req.file.size)} with ${cfg.model}`);
 ...
 */
export async function OpenAISpeech(req, cfg, options) {
 ...
 */
export async function OpenAIImageGeneration(req, cfg, options) {
    ...
    if (isDallE3) {
        if (body.size === "portrait")
            body.size = "1024x1792";
        else if (body.size === "landscape")
            body.size = "1792x1024";
        else if (body.size === "square")
            body.size = "1024x1024";
    }
    else if (isDallE2) {
 ...
 */
export async function OpenAIEmbedder(input, cfg, options) {
 ...
 */
export function LocalOpenAICompatibleModel(providerId, options) {
    return Object.freeze(deleteUndefinedValues({
