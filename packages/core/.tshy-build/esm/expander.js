...
import { finalizeMessages, renderPromptNode } from "./promptdom.js";
 ...
 */
export async function callExpander(prj, r, ev, options, installGlobally) {
    ...
    try {
        ...
        const node = ctx.node;
        const { messages: msgs, images: imgs, errors, schemas: schs, tools: fns, fileMerges: fms, outputProcessors: ops, chatParticipants: cps, fileOutputs: fos, prediction: pred, disposables: mcps, } = await renderPromptNode(modelId, node, {
 ...
 */
export async function expandTemplate(prj, template, options, env) {
    ...
    const renderChatMessages = options.renderChatMessages === true || template.disableChatPreview !== true;
    // finalize options
    ...
    try {
        ...
        for (let i = 0; i < systems.length; ++i) {
            ...
            const sysr = await callExpander(prj, system, mergeEnvVarsWithSystem(env, systemId), { ...options, trace }, false);
            if (sysr.images)
