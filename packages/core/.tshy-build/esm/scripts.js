 ...
 */
export function createScript(name, options) {
 ...
 */
export async function fixPromptDefinitions(project, options) {
...
export async function fixGitHubCopilotInstructions(options) {
    ...
    await writeText(host.path.join(gdir, TYPE_DEFINITION_BASENAME), promptDefinitions[TYPE_DEFINITION_BASENAME]); // Write the TypeScript definition file
    ...
    if (githubCopilotInstructions || docs) {
        ...
        if (!text) {
            ...
            if (!content.ok)
                logVerbose(`failed to fetch ${url}`);
            text = await content.text();
            text = _fullDocsText = collapseNewlines(text.replace(/^!\[\]\(<data:image\/svg\+xml,.*$/gm, "<!-- mermaid diagram -->"));
        }
