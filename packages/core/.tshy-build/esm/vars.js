 ...
 */
export function resolveScriptParametersSchema(prj, script) {
    const res = {
        type: "object",
        properties: {},
    };
 ...
 */
export function systemParameterToVarName(system, name) {
 ...
 */
export function parsePromptParameters(prj, script, optionsVars) {
    const res = {};
    ...
    for (const key in vars) {
        ...
        const t = promptParameterTypeToJSONSchema(p);
        if (t?.type === "number")
            res[key] = normalizeFloat(vars[key]);
        else if (t?.type === "integer")
            res[key] = normalizeInt(vars[key]);
 ...
 */
export function proxifyEnvVars(res) {
    const varsProxy = new Proxy(Object.fromEntries(Object.entries(res).map(([k, v]) => [normalizeVarKey(k), v])), {
        get(target, prop) {
            if (prop === Object.prototype.toString)
                return YAMLStringify(Object.fromEntries(Object.entries(res).map(([k, v]) => [normalizeVarKey(k), v])));
            if (typeof prop === "string")
                return target[normalizeVarKey(prop)];
            return undefined;
        },
        ownKeys(target) {
            return Reflect.ownKeys(target).map((k) => normalizeVarKey(k));
        },
 ...
 */
export function mergeEnvVarsWithSystem(ev, system) {
 ...
 */
export function parametersToVars(parameters) {
    if (!parameters)
        return undefined;
 ...
 */
export function parseOptionsVars(vars, env) {
