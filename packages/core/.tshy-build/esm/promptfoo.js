...
import { CSV_REGEX, HTTPS_REGEX, JSON5_REGEX, MJTS_REGEX, MODEL_PROVIDER_AZURE_OPENAI, MODEL_PROVIDER_AZURE_SERVERLESS_OPENAI, MODEL_PROVIDER_GITHUB, MODEL_PROVIDER_OPENAI, OPENAI_API_BASE, PROMPTFOO_REDTEAM_NUM_TESTS, TEST_CSV_ENTRY_SEPARATOR, XML_REGEX, YAML_REGEX, } from "./constants.js";
 ...
 */
export async function generatePromptFooConfiguration(script, options) {
    ...
    for (const testOrFile of testsAndFiles) {
        if (Array.isArray(testOrFile))
            ...
            if (CSV_REGEX.test(testOrFile)) {
            ...
            else if (JSON5_REGEX.test(testOrFile) ||
                YAML_REGEX.test(testOrFile) ||
                XML_REGEX.test(testOrFile)) {
                const data = arrayify(await runtimeHost.workspace.readData(testOrFile));
                for (const row of data) {
                ...
                const res = await importFile(testOrFile, {
    ...
    for (const test of tests) {
        const v = validateJSONWithSchema(test, testSchema, {
            trace,
        });
        if (v.schemaError)
    ...
    const res = deleteUndefinedValues({
        ...
        tests: arrayify(tests).map(({ description, files, workspaceFiles, vars, rubrics, facts, format = "text", keywords = [], forbidden = [], asserts = [], }) => deleteEmptyValues({
            description,
