...
import { FETCH_RETRY_DEFAULT, FETCH_RETRY_DEFAULT_DEFAULT, FETCH_RETRY_GROWTH_FACTOR, FETCH_RETRY_MAX_DELAY_DEFAULT, FETCH_RETRY_MAX_RETRY_AFTER_DEFAULT, FETCH_RETRY_ON_DEFAULT, } from "./constants.js";
...
export async function createFetch(options) {
    options = options || {};
    const { retries = FETCH_RETRY_DEFAULT, retryOn = FETCH_RETRY_ON_DEFAULT, trace, retryDelay = FETCH_RETRY_DEFAULT_DEFAULT, maxDelay = FETCH_RETRY_MAX_DELAY_DEFAULT, maxRetryAfter = FETCH_RETRY_MAX_RETRY_AFTER_DEFAULT, cancellationToken, } = options;
    dbg(`create fetch`);
    ...
    const crossFetchWithProxy = (url, options) => {
    ...
    dbgr(`retries: %d, retry on: %o, retry delay: %d, max delay: %d, max retry after: %d`, retries, retryOn, retryDelay, maxDelay, maxRetryAfter);
    const fetchRetry = wrapFetch(crossFetchWithProxy, {
        ...
        retryOn: (attempt, error, response) => {
            ...
            if (!isNaN(maxRetryAfter) && retryAfter > maxRetryAfter) {
                dbgr(`retry-after %s exceeds max-retry-after %s, give up`, prettyDuration(retryAfter), prettyDuration(maxRetryAfter));
                return false;
            }
            return true;
        },
        retryDelay: (attempt, error, response) => {
            ...
            const msg = prettyStrings(`retry #${attempt + 1} in ${prettyDuration(delay)}`, `retry after: ${prettyDuration(retryAfter)}`, `max delay: ${prettyDuration(maxDelay)}`, `retry delay: ${prettyDuration(retryDelay)}`, errorMessage(error), statusToMessage(response));
            logVerbose(msg);
...
export async function* iterateBody(r, options) {
