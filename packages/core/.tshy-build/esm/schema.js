 ...
 */
export function JSONSchemaToFunctionParameters(schema) {
 ...
 */
export function JSONSchemaStringifyToTypeScript(schema, options) {
...
export function tryValidateJSONWithSchema(object, options) {
 ...
 */
export function validateFencesWithSchema(fences, schemas, options) {
    const frames = [];
    ...
    for (const fence of fences?.filter(({ language, args }) => args?.schema && (language === "json" || language === "yaml"))) {
        ...
        try {
            if (language === "json")
                data = JSON5parse(val, { repair: true });
 ...
 */
export function JSONSchemaStringify(schema) {
 ...
 */
export function toStrictJSONSchema(schema, options) {
    const { noDefaults, defaultOptional } = options || {};
    const clone = structuredClone(promptParametersSchemaToJSONSchema(schema, { noDefaults }));
    visit(clone);
 ...
 */
export async function JSONSchemaInfer(obj) {
