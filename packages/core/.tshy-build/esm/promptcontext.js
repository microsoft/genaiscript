...
import { createChatGenerationContext } from "./runpromptcontext.js";
 ...
 */
export async function createPromptContext(prj, ev, options, model) {
    ...
    const workspace = {
        ...
        grep: async (query, grepOptions, grepOptions2) => {
            ...
            const grepTrace = trace?.startTraceDetails(`🌐 grep ${HTMLEscape(typeof query === "string" ? query : query.source)} ${glob ? `--glob ${glob}` : ""} ${path || ""}`);
            try {
    ...
    const promptHost = Object.freeze({
        ...
        resolveLanguageModel: async (modelId) => {
            const { configuration } = await resolveModelConnectionInfo({ model: modelId }, {
                token: false,
                trace,
            });
            const res = {
        ...
        cache: async (name) => {
            ...
            const res = createCache(name, { type: "memory" });
            return res;
        },
        ...
        python: async (pyOptions) => await runtimeHost.python({
            trace,
            cancellationToken,
            ...(pyOptions || {}),
        }),
