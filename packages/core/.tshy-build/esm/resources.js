...
const uriResolvers = {
    ...
    vscode: async (dbg, url) => {
        ...
        if (url.host === "vsls-contrib.gistfs" && url.pathname === "/open") {
            ...
            return await uriResolvers.gist(dbg, new URL(`gist://${gist}/${file}`));
        }
        return undefined;
    },
    git: async (dbg, url) => {
        ...
        const files = (await expandFiles([join(cwd, glob)], {
 ...
 */
export async function tryResolveResource(url, options) {
    if (!url)
        return undefined;
