 ...
 */
export function applyModelProviderAliases(id, source) {
 ...
 */
export function applyModelOptions(options, source) {
    dbg(`apply model options from ${source}`, options);
    if (options.provider)
        applyModelProviderAliases(options.provider, source);
    if (options.model)
        runtimeHost.setModelAlias(source, LARGE_MODEL_ID, options.model);
    if (options.smallModel)
        runtimeHost.setModelAlias(source, SMALL_MODEL_ID, options.smallModel);
    if (options.visionModel)
        runtimeHost.setModelAlias(source, VISION_MODEL_ID, options.visionModel);
 ...
 */
export function applyScriptModelAliases(script) {
 ...
 */
export function logModelAliases(options) {
    const { all } = options || {};
