...
import { BOX_RIGHT, BOX_UP_AND_RIGHT, CHAR_ENVELOPE, CHAR_FLOPPY_DISK, CHAR_UP_DOWN_ARROWS, MODEL_PRICINGS, } from "./constants.js";
...
export function estimateCost(modelId, usage) {
    ...
    if (!cost) {
        const m = model.match(/^gpt-(3\.5|4|4o|o1|o3|o4|o4-mini|o1-mini|o1-preview|4o-mini|o3-mini|4\.1|4\.1-mini|4\.1-nano)/);
        if (m) {
            model = m[0];
...
export function estimateImageCost(modelId, usage) {
    if (!modelId || !usage?.total_tokens)
        return undefined;
 ...
 */
export function isCosteable(model) {
...
export class GenerationStats {
        ...
        if (this.chatTurns.length > 1) {
            ...
            try {
                for (const { cached, messages, usage } of this.chatTurns) {
                    trace.item(toStringList(cached ? CHAR_FLOPPY_DISK : "", `${CHAR_ENVELOPE} ${messages.length}`, usage.total_tokens ? `${prettyTokens(usage.total_tokens)}` : undefined));
                }
            }
            finally {
                trace.endDetails();
        ...
        if (this.chatTurns.length > 1) {
            ...
            for (const { messages, usage, model: turnModel, cached } of chatTurns.filter(({ usage }) => usage.total_tokens !== undefined)) {
                ...
                if (cost === undefined && isCosteable(turnModel))
                    unknowns.add(this.model);
                logVerbose(`${indent}  ${cached ? CHAR_FLOPPY_DISK : ""}${toStringList(`${CHAR_ENVELOPE} ${messages.length}`, prettyTokens(usage.total_tokens), prettyCost(cost), prettyTokensPerSecond(usage))}`);
            }
    ...
    addRequestUsage(modelId, req, resp) {
        ...
        logVerbose(`${BOX_UP_AND_RIGHT}${BOX_RIGHT}🏁 ${cached ? CHAR_FLOPPY_DISK : ""} ${modelId} ${CHAR_ENVELOPE} ${messages.length} ${[
