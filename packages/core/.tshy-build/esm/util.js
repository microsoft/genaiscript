 ...
 */
export function toStringList(...token) {
 ...
 */
export function collapseEmptyLines(text) {
 ...
 */
export function relativePath(root, fn) {
    // ignore empty path or urls
    if (!fn || HTTPS_REGEX.test(fn))
        return fn;
 ...
 */
export function ellipseLast(text, length) {
    if (text?.length > length)
