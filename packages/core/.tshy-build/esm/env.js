 ...
 */
export function ollamaParseHostVariable(env) {
    dbg(`ollamaParseHostVariable called with env: ${JSON.stringify(env)}`);
    const s = (env.OLLAMA_HOST || env.OLLAMA_API_BASE || OLLAMA_API_BASE)?.trim();
 ...
 */
export function findEnvVar(env, prefixes, names) {
 ...
 */
export async function parseDefaultsFromEnv(env) {
    ...
    const rx = /^(GENAISCRIPT(_DEFAULT)?_(?<id>[A-Z0-9_-]+)_MODEL|(INPUT_)?MODEL_(?<id2>[A-Z0-9_-]+))$/i;
 ...
 */
export async function parseTokenFromEnv(env, modelId, options) {
    const { resolveToken } = options || {};
    const { provider, model, tag } = parseModelIdentifier(modelId ?? runtimeHost.modelAliases.large.model);
    ...
    if (provider === MODEL_PROVIDER_GITHUB) {
        ...
        const res = findEnvVar(env, "", ["GITHUB_MODELS_TOKEN", ...GITHUB_TOKENS]) || {
            name: undefined,
            value: undefined,
        };
        if (!res?.value) {
            if (resolveToken) {
                const { exitCode, stdout } = await runtimeHost.exec(undefined, "gh", ["auth", "token"], options);
                if (exitCode !== 0)
            ...
            if (!res?.value)
                throw new Error("GITHUB_MODELS_TOKEN, GITHUB_MODELS_TOKEN, GITHUB_TOKEN or GH_TOKEN must be set");
        }
    ...
    if (provider === MODEL_PROVIDER_AZURE_OPENAI) {
        ...
        let base = trimTrailingSlash(env.AZURE_OPENAI_ENDPOINT ||
        ...
        const version = env[`AZURE_OPENAI_API_VERSION_${model.toLocaleUpperCase()}`] ||
    ...
    if (provider === MODEL_PROVIDER_AZURE_SERVERLESS_OPENAI) {
        ...
        const token = env[tokenVar];
        let base = trimTrailingSlash(env.AZURE_SERVERLESS_OPENAI_ENDPOINT || env.AZURE_SERVERLESS_OPENAI_API_ENDPOINT);
        if (!token && !base) {
            return undefined;
        }
        if (token === PLACEHOLDER_API_KEY) {
    ...
    if (provider === MODEL_PROVIDER_AZURE_AI_INFERENCE) {
        ...
        let base = trimTrailingSlash(env.AZURE_AI_INFERENCE_ENDPOINT || env.AZURE_AI_INFERENCE_API_ENDPOINT);
        if (!token && !base) {
            return undefined;
        }
        if (token === PLACEHOLDER_API_KEY) {
    ...
    if (provider === MODEL_PROVIDER_AZURE_SERVERLESS_MODELS) {
        ...
        const token = env[tokenVar]?.trim();
        let base = trimTrailingSlash(env.AZURE_SERVERLESS_MODELS_ENDPOINT || env.AZURE_SERVERLESS_MODELS_API_ENDPOINT);
        if (!token && !base) {
            return undefined;
        }
        if (token === PLACEHOLDER_API_KEY) {
    ...
    if (provider === MODEL_PROVIDER_GOOGLE) {
        ...
        const base = env.GEMINI_API_BASE || env.GOOGLE_API_BASE || GOOGLE_API_BASE;
        if (base === PLACEHOLDER_API_BASE) {
            throw new Error("GEMINI_API_KEY/GOOGLE_API_BASE not configured");
        }
        return {
    ...
    if (provider === MODEL_PROVIDER_ANTHROPIC) {
        ...
        const base = trimTrailingSlash(env.ANTHROPIC_API_BASE) || ANTHROPIC_API_BASE;
    ...
    if (provider === MODEL_PROVIDER_ALIBABA) {
        ...
        const base = env.ALIBABA_API_BASE || env.DASHSCOPE_API_BASE || env.DASHSCOPE_HTTP_BASE_URL || ALIBABA_BASE;
        if (base === PLACEHOLDER_API_BASE) {
    ...
    if (provider === MODEL_PROVIDER_DOCKER_MODEL_RUNNER) {
        ...
        const base = env.DOCKER_MODEL_RUNNER_API_BASE || DOCKER_MODEL_RUNNER_API_BASE;
        if (base === PLACEHOLDER_API_BASE) {
    ...
    if (provider === MODEL_PROVIDER_HUGGINGFACE) {
        ...
        const base = findEnvVar(env, prefixes, BASE_SUFFIX)?.value || HUGGINGFACE_API_BASE;
        if (!URL.canParse(base)) {
            throw new Error(`${base} must be a valid URL`);
        }
        if (!token?.value) {
    ...
    if (provider === MODEL_PROVIDER_DEEPSEEK) {
        ...
        const base = findEnvVar(env, "DEEPSEEK", BASE_SUFFIX)?.value || DEEPSEEK_API_BASE;
        if (!URL.canParse(base)) {
            throw new Error(`${base} must be a valid URL`);
        }
    ...
    if (provider === MODEL_PROVIDER_WHISPERASR) {
        ...
        const base = findEnvVar(env, "WHISPERASR", BASE_SUFFIX)?.value || WHISPERASR_API_BASE;
        if (!URL.canParse(base)) {
            throw new Error(`${base} must be a valid URL`);
        }
        return {
    ...
    if (provider === MODEL_PROVIDER_VLLM) {
        ...
        const base = findEnvVar(env, "VLLM", BASE_SUFFIX)?.value || VLLM_API_BASE;
        if (!URL.canParse(base)) {
            throw new Error(`${base} must be a valid URL`);
        }
        return {
    ...
    if (provider === MODEL_PROVIDER_LLAMAFILE) {
        ...
        const base = findEnvVar(env, "LLAMAFILE", BASE_SUFFIX)?.value || LLAMAFILE_API_BASE;
        if (!URL.canParse(base)) {
            throw new Error(`${base} must be a valid URL`);
        }
        return {
    ...
    if (provider === MODEL_PROVIDER_LITELLM) {
        ...
        const base = findEnvVar(env, "LITELLM", BASE_SUFFIX)?.value || LITELLM_API_BASE;
        if (!URL.canParse(base)) {
            throw new Error(`${base} must be a valid URL`);
        }
        return {
    ...
    if (provider === MODEL_PROVIDER_LMSTUDIO) {
        ...
        const base = findEnvVar(env, "LMSTUDIO", BASE_SUFFIX)?.value || LMSTUDIO_API_BASE;
        if (!URL.canParse(base)) {
            throw new Error(`${base} must be a valid URL`);
        }
        return {
    ...
    if (provider === MODEL_PROVIDER_GITHUB_COPILOT_CHAT) {
        ...
        if (!runtimeHost.clientLanguageModel) {
            throw new Error(`${MODEL_PROVIDER_GITHUB_COPILOT_CHAT} requires Visual Studio Code and GitHub Copilot Chat`);
        }
        return {
            provider,
            model,
    ...
    function cleanAzureBase(b) {
        ...
        const res = trimTrailingSlash(b.replace(/\/openai\/deployments.*$/, "")) + `/openai/deployments`;
        return res;
    }
