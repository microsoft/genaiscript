...
export function dotGenaiscriptPath(...segments) {
    ...
    return resolve(runtimeHost.projectFolder(), GENAISCRIPT_FOLDER, ...segments.map((s) => sanitizeFilename(s)));
}
/**
...
export function getRunDir(scriptId, runId) {
    ...
    const out = dotGenaiscriptPath(RUNS_DIR_NAME, basename(scriptId).replace(GENAI_ANYTS_REGEX, ""), name);
    dbg("run dir: %s", out);
    return out;
}
...
export function getConvertDir(scriptId) {
    ...
    const name = createDatedFolder(runId);
    const out = dotGenaiscriptPath(CONVERTS_DIR_NAME, basename(scriptId).replace(GENAI_ANYTS_REGEX, ""), name);
