...
import { CONVERTS_DIR_NAME, GENAI_ANYTS_REGEX, GENAISCRIPT_FOLDER, RUNS_DIR_NAME, STATS_DIR_NAME, } from "./constants.js";
 ...
 */
export function dotGenaiscriptPath(...segments) {
    return host.resolvePath(host.projectFolder(), GENAISCRIPT_FOLDER, ...segments.map((s) => sanitizeFilename(s)));
}
/**
 ...
 */
export function getRunDir(scriptId, runId) {
    dbg(`run: %s %s`, scriptId, runId);
    const name = createDatedFolder(runId);
    const out = dotGenaiscriptPath(RUNS_DIR_NAME, host.path.basename(scriptId).replace(GENAI_ANYTS_REGEX, ""), name);
    dbg("run dir: %s", out);
    return out;
}
export function getTestDir(runId) {
    dbg(`test: %s`, runId);
    const name = createDatedFolder(runId);
 ...
 */
export function getConvertDir(scriptId) {
    const runId = randomHex(6);
    dbg(`convert: %s %s`, scriptId, runId);
    const name = createDatedFolder(runId);
    const out = dotGenaiscriptPath(CONVERTS_DIR_NAME, host.path.basename(scriptId).replace(GENAI_ANYTS_REGEX, ""), name);
