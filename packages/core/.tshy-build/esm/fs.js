 ...
 */
export function changeext(filename, newext) {
 ...
 */
export async function readText(fn) {
    dbg(`reading file ${fn}`);
 ...
 */
export async function tryReadText(fn) {
    try {
 ...
 */
export async function writeText(fn, content) {
    if (!fn)
 ...
 */
export async function appendText(fn, content) {
    if (!fn)
        throw new Error("filename is required");
    await ensureDir(dirname(fn));
 ...
 */
export async function fileExists(fn) {
 ...
 */
export async function tryStat(fn) {
    try {
 ...
 */
export async function tryReadJSON(fn) {
    try {
        if (!fn)
            return undefined;
        return JSON.parse(await readText(fn));
...
export async function tryReadJSON5(fn) {
    try {
 ...
 */
export async function writeJSON(fn, obj) {
    if (!fn)
        throw new Error("filename is required");
 ...
 */
export async function expandFiles(files, options) {
    ...
    const others = await runtimeHost.findFiles(files.filter((f) => !HTTPS_REGEX.test(f)), {
 ...
 */
export async function expandFileOrWorkspaceFiles(files) {
 ...
 */
export function filePathOrUrlToWorkspaceFile(f) {
