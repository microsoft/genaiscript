...
import { tidy, sliceTail, sliceHead, sliceSample, select, distinct, arrange, } from "@tidyjs/tidy";
 ...
 */
export function sliceData(rows, options = {}) {
    if (!rows)
        return rows;
    ...
    if (options.sliceHead > 0)
        rows = tidy(rows, sliceHead(options.sliceHead));
    // Check if the tail of rows is to be sliced and apply slicing
    if (options.sliceTail > 0)
        rows = tidy(rows, sliceTail(options.sliceTail));
    return rows;
}
