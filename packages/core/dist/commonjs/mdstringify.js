 ...
 */
function markdownStringify(obj, options) {
    const seen = new Set();
    ...
    const render = (obj, depth) => {
        ...
        if (Array.isArray(obj)) {
            ...
            if (depth <= headings) {
                return entries
                    .map((kv) => `\n${"#".repeat(headingLevel + depth)} ${(0, inflection_js_1.titleize)(kv[0])}\n${kv[1]}`)
                    .join("\n");
            }
            ...
            if (quoteValues) {
                if (obj.includes("\n"))
                    return (0, mkmd_js_1.fenceMD)(obj);
            ...
            return quoteValues ? `\`${String(obj).replace(/`/g, "\\`")}\`` : String(obj);
    };
    return render(obj, 0) + "\n";
}
//# sourceMappingURL=mdstringify.js.map