...
exports.isSARIFFilename = isSARIFFilename;
 ...
 */
async function convertDiagnosticsToSARIF(template, issues) {
    const { SarifBuilder, SarifRunBuilder, SarifResultBuilder, SarifRuleBuilder } = await import("node-sarif-builder");
    // Initialize a SARIF run with tool driver information
    const sarifRunBuilder = new SarifRunBuilder().initSimple({
        toolDriverName: constants_js_1.SARIFF_BUILDER_TOOL_DRIVER_NAME,
        toolDriverVersion: version_js_1.CORE_VERSION,
        url: constants_js_1.SARIFF_BUILDER_URL,
    });
    // Initialize a SARIF rule based on the provided template
    const sarifRuleBuiler = new SarifRuleBuilder().initSimple({
        ruleId: constants_js_1.SARIFF_RULEID_PREFIX + template.id, // Unique rule identifier
    ...
    for (const issue of issues) {
        ...
        sarifResultBuilder.initSimple({
            ...
            fileUri: (0, node_path_1.relative)(process.cwd(), issue.filename).replace(/\\/g, "/"), // Convert file path to a relative URI
