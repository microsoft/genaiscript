...
exports.serializeLogProb = serializeLogProb;
exports.serializeChunkChoiceToLogProbs = serializeChunkChoiceToLogProbs;
exports.renderLogprob = renderLogprob;
exports.logprobColor = logprobColor;
exports.rgbToCss = rgbToCss;
exports.logprobToMarkdown = logprobToMarkdown;
exports.topLogprobsToMarkdown = topLogprobsToMarkdown;
exports.computePerplexity = computePerplexity;
exports.computeStructuralUncertainty = computeStructuralUncertainty;
 ...
 */
function serializeLogProb(content) {
 ...
 */
function serializeChunkChoiceToLogProbs(choice) {
    const { delta, logprobs } = choice;
    if (logprobs?.content)
        return logprobs.content.map(serializeLogProb);
    else
        return [
            {
                token: delta.content || "",
...
function logprobToPercent(value) {
    if (value === undefined)
        return NaN;
    const linearProbability = (0, precision_js_1.roundWithPrecision)(Math.exp(value) * 100, 2);
 ...
 */
function renderLogprob(logprob) {
    return logprob === undefined || isNaN(logprob)
        ? `--`
        : `${logprobToPercent(logprob)}% (${(0, precision_js_1.roundWithPrecision)(logprob, 2)})`;
}
/**
 ...
 */
function logprobColor(logprob, options) {
 ...
 */
function logprobToMarkdown(value, options) {
    const { token, logprob, entropy } = value;
    const c = rgbToCss(logprobColor(value, options));
    const title = options?.entropy ? (0, precision_js_1.roundWithPrecision)(entropy, 2) : renderLogprob(logprob);
    let text = (0, html_escaper_1.escape)(token).replace(/</g, "&lt;").replace(/>/g, "&gt;");
    if (options?.eatSpaces)
        text = text.replace(/\n/g, " ");
    else
 ...
 */
function topLogprobsToMarkdown(value, options) {
 ...
 */
function computeStructuralUncertainty(logprobs) {
    if (!logprobs?.length)
        return undefined;
    const vs = logprobs
...
//# sourceMappingURL=logprob.js.map