...
const ItemSelector_js_1 = require("./ItemSelector.js");
const debug_js_1 = require("../debug.js");
const path_1 = require("path");
const dbg = (0, debug_js_1.genaiscriptDebug)("vector:index");
/**
...
class LocalIndex {
    ...
    async createIndex(config = { version: 1 }) {
        ...
        try {
            ...
            await fs.writeFile((0, path_1.join)(this._folderPath, this._indexName), JSON.stringify(this._data));
        }
        catch (err) {
            dbg(err);
            await this.deleteIndex();
    ...
    async endUpdate() {
        ...
        try {
            // Save index
            await fs.writeFile((0, path_1.join)(this._folderPath, this._indexName), JSON.stringify(this._update));
    ...
    async isIndexCreated() {
        try {
            await fs.access((0, path_1.join)(this._folderPath, this.indexName));
            return true;
        }
        catch (err) {
            dbg(err);
    ...
    async listItemsByMetadata(filter) {
        await this.loadIndexData();
        return this._data.items.filter((i) => ItemSelector_js_1.ItemSelector.select(i.metadata, filter));
    }
    /**
    ...
    async queryItems(vector, topK, filter) {
        ...
        if (filter) {
            items = items.filter((i) => ItemSelector_js_1.ItemSelector.select(i.metadata, filter));
        }
        // Calculate distances
        const norm = ItemSelector_js_1.ItemSelector.normalize(vector);
        const distances = [];
        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            const distance = ItemSelector_js_1.ItemSelector.normalizedCosineSimilarity(vector, norm, item.vector, item.norm);
        ...
        for (const item of top) {
            if (item.item.metadataFile) {
                const metadataPath = (0, path_1.join)(this._folderPath, item.item.metadataFile);
    ...
    async loadIndexData() {
        ...
        const data = await fs.readFile((0, path_1.join)(this._folderPath, this.indexName));
    ...
    async addItemToUpdate(item, unique) {
        ...
        if (this._update.metadata_config.indexed &&
            ...
            const metadataPath = (0, path_1.join)(this._folderPath, metadataFile);
        ...
        const newItem = {
            ...
            metadata: metadata,
            vector: item.vector,
            norm: ItemSelector_js_1.ItemSelector.normalize(item.vector),
        };
        if (metadataFile) {
