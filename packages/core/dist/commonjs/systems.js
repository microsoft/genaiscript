...
exports.resolveSystems = resolveSystems;
exports.addFallbackToolSystems = addFallbackToolSystems;
...
const dbg = (0, debug_js_1.genaiscriptDebug)("systems");
 ...
 */
function resolveSystems(prj, script, resolvedTools) {
    const { jsSource, responseType, responseSchema, systemSafety, mcpServers, mcpAgentServers } = script;
    // Initialize systems array from script.system, converting to array if necessary using arrayify utility
    let systems = (0, cleaners_js_1.arrayify)(script.system).filter((s) => typeof s === "string");
    const systemInstances = (0, cleaners_js_1.arrayify)(script.system).filter((s) => typeof s === "object");
    const excludedSystem = (0, cleaners_js_1.arrayify)(script.excludedSystem);
    const tools = (0, cleaners_js_1.arrayify)(script.tools);
    const dataMode = responseSchema || (responseType && responseType !== "markdown" && responseType !== "text");
    const safeties = [
    ...
    systems = (0, es_toolkit_1.uniq)(systems);
    // now compute system instances
    const res = [...systems.map((id) => ({ id })), ...systemInstances];
    dbgr(`resolved %O`, res);
    return res;
}
/**
 ...
 */
function addFallbackToolSystems(systems, tools, options, genOptions) {
    if (!tools?.length || systems.find(({ id }) => id === "system.tool_calls")) {
        dbg(`no tools or fallback tools found, skip fallback tools`);
        return false;
    }
    const supported = (0, tools_js_1.isToolsSupported)(options?.model || genOptions?.model);
    const fallbackTools = supported === false || options?.fallbackTools || genOptions?.fallbackTools;
    if (fallbackTools) {
        dbg(`adding fallback tools to systems`, (0, cleaners_js_2.deleteUndefinedValues)({
            supported,
 ...
 */
function resolveSystemFromTools(prj, tool) {
    const system = prj.scripts.filter((t) => t.isSystem && t.defTools?.find((to) => to.id.startsWith(tool)));
 ...
 */
function resolveTools(prj, systems, tools) {
    const { scripts: scripts } = prj;
    const toolScripts = (0, es_toolkit_1.uniq)([
