...
class WebSocketClient extends EventTarget {
    ...
    reconnect() {
        ...
        this.dispatchEvent(new Event(constants_js_1.RECONNECT));
        this._ws = undefined;
        ...
        }, constants_js_1.CLIENT_RECONNECT_DELAY);
    }
    connect() {
        this._error = undefined;
        this._ws = new WebSocket(this.url);
        this._ws.addEventListener(constants_js_1.OPEN, () => {
            // clear counter
            ...
            while (this._ws?.readyState === WebSocket.OPEN && (m = this._pendingMessages.pop()))
                this._ws.send(m);
            this.dispatchEvent(new Event(constants_js_1.OPEN));
            this.dispatchChange();
        }, false);
        this._ws.addEventListener(constants_js_1.ERROR, (ev) => {
            this.reconnect();
            this.dispatchChange();
        }, false);
        this._ws.addEventListener(constants_js_1.CLOSE, 
        // CloseEvent not defined in electron
        (ev) => {
            const reason = ev.reason || "websocket closed";
            this.cancel(reason);
            this.dispatchEvent(new Event(constants_js_1.CLOSE));
            this.dispatchChange();
            this.reconnect();
        }, false);
        this._ws.addEventListener(constants_js_1.MESSAGE, (async (e) => {
            const event = e;
            ...
            // not a response
            this.dispatchEvent(new MessageEvent(constants_js_1.MESSAGE, {
                data,
            }));
        }), false);
        this.dispatchEvent(new Event(constants_js_1.CONNECT));
    }
    queue(msg, options) {
        const { reuse } = options || {};
        if (reuse) {
    ...
    async startScript(runId, script, files, options) {
        this.dispatchEvent(new Event(constants_js_1.QUEUE_SCRIPT_START));
        return this.queue({
            type: "script.start",
            runId,
            script,
