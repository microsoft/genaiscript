...
import { BOX_DOWN_AND_RIGHT, BOX_LEFT_AND_DOWN, BOX_LEFT_AND_UP, BOX_RIGHT, BOX_UP_AND_DOWN, BOX_UP_AND_RIGHT, CHAR_DOWN_ARROW, CHAR_UP_ARROW, CHAR_UP_DOWN_ARROWS, CONSOLE_COLOR_DEBUG, IMAGE_DETAIL_HIGH_HEIGHT, IMAGE_DETAIL_HIGH_WIDTH, IMAGE_DETAIL_LOW_HEIGHT, IMAGE_DETAIL_LOW_WIDTH, } from "./constants.js";
...
async function prepare(url, options) {
    ...
    if (detail === "low") {
        ...
        contain(img, Math.min(img.width, IMAGE_DETAIL_LOW_WIDTH), Math.min(img.height, IMAGE_DETAIL_LOW_HEIGHT), HorizontalAlign.CENTER | VerticalAlign.MIDDLE);
    }
    else if (detail !== "original") {
        dbg(`setting image detail to low`);
        contain(img, IMAGE_DETAIL_HIGH_WIDTH, IMAGE_DETAIL_HIGH_HEIGHT, HorizontalAlign.CENTER | VerticalAlign.MIDDLE);
    }
    return img;
}
 ...
 */
export async function imageTransform(url, options) {
    const { mime } = options || {};
 ...
 */
export async function imageEncodeForLLM(url, options) {
 ...
 */
export async function imageTileEncodeForLLM(urls, options) {
    if (urls.length === 0) {
    ...
    contain(canvas, IMAGE_DETAIL_HIGH_WIDTH, IMAGE_DETAIL_HIGH_HEIGHT, HorizontalAlign.CENTER | VerticalAlign.MIDDLE);
 ...
 */
export async function renderImageToTerminal(url, options) {
    ...
    const res = [
        wrapColor(CONSOLE_COLOR_DEBUG, `${BOX_DOWN_AND_RIGHT}${BOX_RIGHT}` +
            title +
    ...
    const usageStr = usage
        ...
        : "";
    res.push(wrapColor(CONSOLE_COLOR_DEBUG, BOX_UP_AND_RIGHT +
        usageStr +
