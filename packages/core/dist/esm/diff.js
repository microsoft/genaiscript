 ...
 */
export function diffParse(input) {
 ...
 */
export function diffResolve(input) {
 ...
 */
export function tryDiffParse(diff) {
    try {
 ...
 */
export function diffCreatePatch(left, right, options) {
    if (typeof left === "string")
        left = { filename: "left", content: left };
    if (typeof right === "string")
        right = { filename: "right", content: right };
    const res = createTwoFilesPatch(left?.filename || "", right?.filename || "", left?.content || "", right?.content || "", undefined, undefined, {
        ignoreCase: true,
 ...
 */
export function diffFindChunk(file, range, diff) {
    // line is zero-based!
