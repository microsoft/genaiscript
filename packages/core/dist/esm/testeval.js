...
export async function evaluateTestResult(config, result) {
    ...
    for (const assert of arrayify(asserts)) {
        ...
        switch (type) {
            ...
            case "contains-all": {
                const { value } = assert;
                passed = arrayify(value).every((v) => transformedUpperText.includes(v.toLocaleUpperCase()));
                break;
            }
            ...
            case "contains-any": {
                const { value } = assert;
                passed = arrayify(value).some((v) => transformedUpperText.includes(v.toLocaleUpperCase()));
                break;
            }
