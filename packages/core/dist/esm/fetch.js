...
import { FETCH_RETRY_DEFAULT, FETCH_RETRY_DELAY_DEFAULT, FETCH_RETRY_GROWTH_FACTOR, FETCH_RETRY_MAX_DELAY_DEFAULT, FETCH_RETRY_MAX_RETRY_AFTER_DEFAULT, FETCH_RETRY_MIN_DELAY_DEFAULT, FETCH_RETRY_ON_DEFAULT, } from "./constants.js";
...
export async function createFetch(options) {
    const { retries = FETCH_RETRY_DEFAULT, retryOn = FETCH_RETRY_ON_DEFAULT, trace, retryDelay = FETCH_RETRY_DELAY_DEFAULT, maxDelay = FETCH_RETRY_MAX_DELAY_DEFAULT, maxRetryAfter = FETCH_RETRY_MAX_RETRY_AFTER_DEFAULT, cancellationToken, } = options || {};
    ...
    const fetchRetry = wrapFetch(crossFetchWithProxy, {
        ...
        retryDelay: (attempt, error, response) => {
            ...
            if (!isNaN(retryAfter)) {
                ...
                delay = Math.max(minDelay, Math.min(maxDelay, Math.ceil(Math.pow(FETCH_RETRY_GROWTH_FACTOR, attempt) * Math.max(retryDelay, minDelay)) *
                    (1 + Math.random() / 20)));
