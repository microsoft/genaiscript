...
export function installGlobals() {
    ...
    glb.MD = Object.freeze({
        ...
        updateFrontmatter: (text, frontmatter, format) => updateFrontmatter(text, frontmatter, { format }), // Update frontmatter in markdown
    ...
    glb.tokenizers = Object.freeze({
        ...
        count: async (text, options) => {
            ...
            if (options?.approximate)
                return approximateTokens(text, { encoder });
 ...
 */
export function installGlobalPromptContext(ctx) {
    const glb = resolveGlobal(); // Get the global context
