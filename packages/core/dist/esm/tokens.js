...
import { ESTIMATE_TOKEN_OVERHEAD, MAX_STRING_LENGTH_USE_TOKENIZER_FOR_APPROXIMATION, MAX_TOKENS_ELLIPSE, PROMPT_DOM_TRUNCATE_ATTEMPTS, TOKEN_TRUNCATION_THRESHOLD, } from "./constants.js";
 ...
 */
export function approximateTokens(text, options) {
    if (!text)
        return 0;
    const { overcount = 0, encoder } = options || {};
    dbg(`approximate %d chars, encoder: %o`, text.length, !!encoder);
    if (encoder && text.length < MAX_STRING_LENGTH_USE_TOKENIZER_FOR_APPROXIMATION)
 ...
 */
export function estimateTokens(text, encoder) {
 ...
 * /NO P/
 */
export function truncateTextToTokens(content, maxTokens, encoder, options) {
    ...
    while (Math.abs(left - right) > threshold && attempts++ < PROMPT_DOM_TRUNCATE_ATTEMPTS) {
