...
async function tryImportPdfjs() {
    ...
    standardFontDataUrl = pathToFileURL(workerSrc.replace("build/pdf.worker.min.mjs", "standard_fonts/")).toString();
...
async function PDFTryParse(fileOrUrl, content, options) {
    const { cancellationToken, disableCleanup, trace, renderAsImage, scale = PDF_SCALE, cache, useSystemFonts, } = options || {};
    ...
    const readCache = async () => {
        ...
        try {
            const res = JSON.parse(await readFile(resFilename, {
                encoding: "utf-8",
            }));
    ...
    try {
        ...
        for (let i = 0; i < numPages; i++) {
            ...
            p.figures = figures;
            logVerbose(`pdf: extracted ${fileOrUrl || ""} page ${i + 1} / ${numPages}, ${p.figures.length ? `${p.figures.length} figures` : ""}`);
        }
    ...
    async function decodeImage(pageIndex, img, createCanvas, imageObj, folder) {
        ...
        const buffer = canvas.toBuffer("image/png");
        const fn = join(folder, `page-${pageIndex}-${imageObj.replace(INVALID_FILENAME_REGEX, "")}.png`);
 ...
 */
export async function parsePdf(filenameOrBuffer, options) {
