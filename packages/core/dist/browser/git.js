...
export class GitClient {
    ...
    async diff(options) {
        ...
        const paths = arrayify(options?.paths, { filterEmpty: true });
        const excludedPaths = await this.resolveExcludedPaths(options);
        const { staged, base, head, unified, askStageOnEmpty, nameOnly, maxTokensFullDiff = GIT_DIFF_MAX_TOKENS, llmify, algorithm = "minimal", extras, } = options || {};
        const args = ["diff"];
        if (staged) {
...
`;
    ...
    async shallowClone(repository, options) {
        ...
        if (!directory) {
            ...
            directory = dotGenaiscriptPath("git", ...url.pathname.split(/\//g).filter((s) => !!s), branch || `HEAD`, sha);
        }
