 ...
 */
export function parseKeyValuePair(text) {
    const m = /[=:]/.exec(text);
    return m ? { [text.slice(0, m.index)]: unquote(text.slice(m.index + 1)) } : {};
}
/**
        ...
        if (currFence) {
            ...
            if (fence.fence && fence.args["file"]) {
                ...
                if (start.fence && line.endsWith(":")) {
                    currLbl = (unquote(line.slice(0, -1)) + " " + (start.args["file"] || "")).trim();
                    ...
                    currLbl = unquote(m[1]) + " " + (start.args["file"] || unquote(m[2]));
                    currFence = start.fence;
                    currLanguage = start.language || "";
                    currArgs = start.args;
                    i++;
 ...
 */
export function findFirstDataFence(fences) {
    const { content, language } = fences?.find((f) => f.content && !f.label && (f.language === "yaml" || f.language === "json")) || {};
    if (language === "yaml" || language === "yml")
        return YAMLTryParse(content);
    else if (language === "json")
        return JSON5TryParse(content);
    return undefined;
}
/**
 ...
 */
export function renderFencedVariables(vars) {
    return vars
