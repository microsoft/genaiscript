 ...
 */
export function parseLLMDiffs(text) {
 ...
 */
export function applyLLMDiff(source, chunks) {
 ...
 */
export function applyLLMPatch(source, chunks) {
    if (!chunks?.length || !source)
        return source;
    const lines = source.split("\n");
 ...
 * @returns The LLMDiff formatted string or undefined if parsing fails.
 */
export function llmifyDiff(diff) {
    if (!diff)
        return diff;
