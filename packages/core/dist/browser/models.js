...
export function normalizeModelIdentifier(id) {
    if (!id)
        return "";
...
export function areModelsSame(l, r) {
    if (!l && !r)
        return true;
    if (!l || !r)
        return false;
 ...
 */
export function traceLanguageModelConnection(trace, options, connectionToken) {
    if (!trace)
        return;
    ...
    try {
        ...
        if (choices.length)
            trace.itemValue(`choices`, choices
                .map((c) => typeof c === "string" ? c : `${c.token} - ${roundWithPrecision(c.weight, 2)}`)
                .join(","));
        ...
        Object.entries(runtimeHost.modelAliases).forEach(([key, value]) => trace.itemValue(key, toStringList(`\`${value.model}\``, isNaN(value.temperature) ? undefined : `temperature: \`${value.temperature}\``, `source: \`${value.source}\``)));
        trace.endDetails();
    }
    finally {
        trace.endDetails();
 ...
 */
export async function resolveModelConnectionInfo(conn, options) {
