...
import { XLSX_REGEX, CSV_REGEX, INI_REGEX, TOML_REGEX, JSON5_REGEX, YAML_REGEX, XML_REGEX, MD_REGEX, MDX_REGEX, JSONL_REGEX, } from "./constants.js";
 ...
 */
export async function dataTryParse(file, options) {
    await resolveFileContent(file);
    const runtimeHost = resolveRuntimeHost();
    const { filename, content, encoding } = file;
    let data;
    if (XLSX_REGEX.test(filename))
        data = await XLSXParse(encoding === "base64" ? fromBase64(content) : await runtimeHost.readFile(filename));
    else {
        if (CSV_REGEX.test(filename))
            data = CSVTryParse(content, options);
        else if (INI_REGEX.test(filename))
            data = INITryParse(content, options);
        else if (TOML_REGEX.test(filename))
            data = TOMLTryParse(content);
        else if (JSON5_REGEX.test(filename))
            data = JSON5TryParse(content, { repair: true });
        else if (YAML_REGEX.test(filename))
            data = YAMLTryParse(content);
        else if (XML_REGEX.test(filename))
            data = await XMLTryParse(content, options);
        else if (JSONL_REGEX.test(filename))
            data = JSONLTryParse(content);
