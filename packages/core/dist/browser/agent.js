...
export function agentCreateCache(options) {
    const cache = createCache(AGENT_MEMORY_CACHE_NAME, {
        type: "memory",
 ...
 */
export async function agentQueryMemory(cache, ctx, query) {
    if (!query)
        return undefined;
    ...
    if (!res.error)
        ...
        dbg(`error: ${errorMessage(res.error)}`);
 ...
 */
export async function agentAddMemory(cache, agent, query, text, options) {
    const { trace } = options || {};
    ...
    trace?.detailsFenced(`🧠 agent memory: ${HTMLEscape(query)}`, HTMLEscape(prettifyMarkdown(cachedValue.answer)), "markdown");
}
 ...
 */
export async function traceAgentMemory(options) {
    const { trace } = options || {};
    if (!trace)
        return;
    ...
    if (memories?.length) {
        try {
            ...
            memories
                .reverse()
                .forEach(({ agent, query, answer }) => trace?.detailsFenced(`👤 ${agent}: ${HTMLEscape(query)}`, HTMLEscape(prettifyMarkdown(answer)), "markdown"));
        }
        finally {
            trace?.endDetails();
        }
