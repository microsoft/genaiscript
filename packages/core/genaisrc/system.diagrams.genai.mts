...
`;
  ...
  // Create the repair function once
  let repairFunction: any = null;
  
  defChatParticipant(async (ctx, messages, assistantText) => {
    try {
      // Create the repair function on first use
      if (!repairFunction) {
        const { createMermaidRepairChatParticipant } = await import("@genaiscript/plugin-mermaid");
        repairFunction = createMermaidRepairChatParticipant({
          maxRepairs: repair,
          logger: dbg,
        });
      }
      
      return await repairFunction(ctx, messages, assistantText);
    } catch (e) {
      dbg(`failed to load mermaid repair: %s`, e);
      return;
    }
  });
}
