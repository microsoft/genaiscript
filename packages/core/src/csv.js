 ...
 */
export function CSVTryParse(text, options) {
    const { trace } = options || {};
    try {
        if (!text)
 ...
 */
export function CSVStringify(csv, options) {
    if (!csv)
 ...
 */
export function dataToMarkdownTable(csv, options) {
    if (!csv?.length)
        return ""; // Return empty string if CSV is empty
 ...
 */
export function headersToMarkdownTableSeparator(headers) {
 ...
 */
export function headersToMarkdownTableHead(headers) {
    return `|${headers.join("|")}|\n`;
}
/**
 ...
 */
export function objectToMarkdownTableRow(row, headers, options) {
 ...
 */
export function CSVChunk(rows, size) {
