 ...
 */
export function arrayify(a, options) {
 ...
 */
export function deleteUndefinedValues(o) {
 ...
 */
export function deleteEmptyValues(o) {
    if (typeof o === "object")
        for (const k in o) {
            const v = o[k];
 ...
 */
export function normalizeFloat(s) {
    if (typeof s === "string") {
 ...
 */
export function normalizeInt(s) {
    if (s === undefined)
        return undefined;
    if (typeof s === "string") {
        const f = parseInt(s);
 ...
 */
export function normalizeBoolean(s) {
...
export function ensureHeadSlash(s) {
    if (s?.startsWith("/"))
        return s;
    return "/" + s;
}
 ...
 */
export function normalizeVarKey(key) {
 ...
 */
export function unmarkdown(text) {
 ...
 */
export function collapseNewlines(res) {
 ...
 */
export function encodeIDs(text, options) {
    const { matcher = /\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b/gi, prefix = "id", open = "{|", close = "|}", } = options || {};
    const ids = {};
    let idCounter = 0;
    ...
    const drx = new RegExp(`${escapeRegExp(open)}${prefix}(\\d+)${escapeRegExp(close)}`, "g");
    const decode = (text) => text?.replace(drx, (encoded) => ids[encoded]);
