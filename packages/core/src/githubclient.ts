...
export function generatedByFooter(
    ...
    const by = script ? ` by ${link(script.id, info.runUrl)}` : ""
    return `\n\n> AI-generated content${by} may be incorrect. Use reactions to eval.\n\n`
}
...
export class GitHubClient implements GitHub {
    ...
    async createIssueComment(
        ...
        // cleanup markdown for gfm
        if (options?.prettifyMarkdown) body = prettifyMarkdown(body)
        // update body with generated by footer
        if (aiDisclaimer) body += generatedByFooter(undefined, {})
