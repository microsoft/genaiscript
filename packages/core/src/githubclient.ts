...
import { createGitHubWorkspace } from "./githubworkspace.js";
...
export async function githubParseEnv(
  ...
  dbg(
    ...
    Object.fromEntries(Object.entries(res).map(([k, v]) => [k, k === "token" ? "***" : v])),
  );
 ...
 */
export function mergeDescription(commentTag: string, body: string, text: string) {
  body = body ?? "";
...
async function githubCreatePullRequestReview(
  ...
  info: Pick<
    ...
    "apiUrl" | "repository" | "issue" | "runUrl" | "commitSha" | "owner" | "repo"
  >,
  token: string,
  annotation: Diagnostic,
  existingComments: {
...
export class GitHubClient implements GitHub {
  ...
  constructor(info: Pick<GithubConnectionInfo, "owner" | "repo">) {
    ...
    this._workspace = createGitHubWorkspace();
  }
  ...
  async getOrCreateRef(
    ...
    if (orphaned) {
      ...
      const { data: blob } = await client.git.createBlob({
        ...
        repo,
        content: Buffer.from(
          typeof orphaned === orphaned ? orphaned : `Orphaned branch created by GenAIScript.`,
        ).toString("base64"),
        encoding: "base64",
      });
  ...
  async uploadAsset(file: BufferLike, options?: { branchName?: string }): Promise<string> {
      ...
      files: Object.values(files).map(
        ({ filename, content, size, encoding }) =>
          deleteUndefinedValues({
            ...
            content,
            encoding:
              encoding === "utf-8" ? undefined : encoding === "base64" ? "base64" : undefined,
            size,
          }) satisfies WorkspaceFile,
      ),
    } satisfies GitHubGist;
  ...
  async createIssueComment(issue_number: number | string, body: string): Promise<GitHubComment> {
    issue_number = normalizeInt(issue_number);
    const { client, owner, repo } = await this.api();
  ...
  async getPullRequest(pull_number?: number | string): Promise<GitHubPullRequest> {
  ...
  async listReleases(options?: GitHubPaginationOptions): Promise<GitHubRelease[]> {
    const { client, owner, repo } = await this.api();
  ...
  async downloadArtifactFiles(artifactId: number | string): Promise<WorkspaceFile[]> {
    const { client, owner, repo } = await this.api();
   ...
   * @param jobId
   */
  async downloadWorkflowJobLog(job_id: number, options?: { llmify?: boolean }): Promise<string> {
    const { client, owner, repo } = await this.api();
    const { url: logs_url } = await client.rest.actions.downloadJobLogsForWorkflowRun({
      owner,
      repo,
  ...
  async listIssueLabels(issueNumber?: string | number): Promise<GitHubLabel[]> {
    const { client, owner, repo } = await this.api();
