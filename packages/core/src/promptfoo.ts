...
export async function generatePromptFooConfiguration(
  ...
  const redteam: Partial<PromptRedteam> = options?.redteam ? script.redteam || {} : undefined;
  ...
  for (const testOrFile of testsAndFiles) {
    if (Array.isArray(testOrFile)) tests.push(...testOrFile);
      ...
      if (CSV_REGEX.test(testOrFile)) {
        const data: any[] = await runtimeHost.workspace.readCSV(testOrFile, {
          repair: false,
        });
        if (!data.length) {
        ...
        const data = arrayify(await runtimeHost.workspace.readData(testOrFile)) as (
          | string
          | PromptTest
        )[];
        for (const row of data) {
