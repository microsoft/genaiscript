...
import { convertGitHubCopilotMcpConfig, readGitHubCopilotMcpConfig } from "./configmcp.js";
...
export function mergeHostConfigs(
  ...
  return deleteEmptyValues({
    ...
    secretPatterns: structuralMerge(config?.secretPatterns || {}, parsed?.secretPatterns || {}),
    mcpServers: structuralMerge(config?.mcpServers || {}, parsed?.mcpServers || {}),
  });
}
...
async function resolveGlobalConfiguration(
  ...
  // Read GitHub Copilot MCP configurations from .vscode/mcp.json files
  dbg("reading GitHub Copilot MCP configurations");
  const copilotMcpServers = await readGitHubCopilotMcpConfig(dirs);
  if (Object.keys(copilotMcpServers).length > 0) {
    dbg(`found ${Object.keys(copilotMcpServers).length} MCP servers from GitHub Copilot configuration`);
    config = mergeHostConfigs(config, { mcpServers: copilotMcpServers });
  }
