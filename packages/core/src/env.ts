...
export async function parseDefaultsFromEnv(env: Record<string, string>) {
  ...
  const rx = /^GENAISCRIPT(_DEFAULT)?_((?<id>[A-Z0-9_\-]+)_MODEL|MODEL_(?<id2>[A-Z0-9_\-]+))$/i;
...
export async function parseTokenFromEnv(
  ...
  if (provider === MODEL_PROVIDER_AZURE_SERVERLESS_OPENAI) {
    ...
    const token = env[tokenVar];
    let base = trimTrailingSlash(
      env.AZURE_SERVERLESS_OPENAI_ENDPOINT || env.AZURE_SERVERLESS_OPENAI_API_ENDPOINT,
    );
    if (!token && !base) {
      return undefined;
    }
  ...
  if (provider === MODEL_PROVIDER_AZURE_AI_INFERENCE) {
    ...
    let base = trimTrailingSlash(
      env.AZURE_AI_INFERENCE_ENDPOINT || env.AZURE_AI_INFERENCE_API_ENDPOINT,
    );
    if (!token && !base) {
      return undefined;
    }
  ...
  if (provider === MODEL_PROVIDER_AZURE_SERVERLESS_MODELS) {
    ...
    const token = env[tokenVar]?.trim();
    let base = trimTrailingSlash(
      env.AZURE_SERVERLESS_MODELS_ENDPOINT || env.AZURE_SERVERLESS_MODELS_API_ENDPOINT,
    );
    if (!token && !base) {
      return undefined;
    }
  ...
  if (provider === MODEL_PROVIDER_ALIBABA) {
    ...
    const base =
      env.ALIBABA_API_BASE || env.DASHSCOPE_API_BASE || env.DASHSCOPE_HTTP_BASE_URL || ALIBABA_BASE;
    if (base === PLACEHOLDER_API_BASE) {
