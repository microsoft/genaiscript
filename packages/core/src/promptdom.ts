 ...
 */
export function toDefRefName(name: string, options: FenceFormatOptions): string {
...
export function createDefDiff(
  ...
  if (left?.content === undefined) left = { filename: "", content: YAMLStringify(left) };
  if (right?.content === undefined) right = { filename: "", content: YAMLStringify(right) };
...
function renderDefNode(def: PromptDefNode): string {
  ...
  if (name && fenceFormat === "xml") {
    ...
    const fence =
      language === "markdown" || language === "mdx" ? MARKDOWN_PROMPT_FENCE : PROMPT_FENCE;
    let dfence =
      /\.mdx?$/i.test(filename) || content?.includes(fence) ? MARKDOWN_PROMPT_FENCE : fence;
 ...
 */
export function createOutputProcessor(fn: PromptOutputProcessorHandler): PromptOutputProcessorNode {
  assert(fn !== undefined);
 ...
 */
export function createChatParticipant(participant: ChatParticipant): PromptChatParticipantNode {
...
function haveSameKeysAndSimpleValues(data: object[]): boolean {
  ...
  return data.slice(1).every((obj) => {
    ...
    return (
      ...
      headers.every(
        (h, i) =>
          keys[i][0] === h[0] && /^(string|number|boolean|null|undefined)$/.test(typeof keys[i][1]),
      )
    );
  });
}
...
export function appendChild(parent: PromptNode, ...children: PromptNode[]): void {
  if (!parent.children) {
    parent.children = [];
  }
  ...
  await visitNode(root, {
    ...
    stringTemplate: async (n) => {
      ...
      try {
        ...
        if (n.transforms?.length)
          for (const transform of n.transforms) value = await transform(value);
        n.resolved = n.preview = value;
        n.tokens = approximateTokens(value);
      } catch (e) {
        n.error = e;
    ...
    importTemplate: async (n) => {
      try {
        ...
        n.children = [];
        n.preview = "";
        const fs: WorkspaceFile[] = await expandFileOrWorkspaceFiles(arrayify(files));
...
async function resolveImportPrompty(
  ...
  for (const message of messages) {
    ...
    if (message.role === "assistant") n.children.push(createAssistantNode(txt));
    else if (message.role === "system") n.children.push(createSystemNode(txt));
  ...
  await visitNode(root, {
    node: (n) => {
      ...
      const value = toStringList(
        n.tokens ? `${n.tokens}${n.maxTokens ? `/${n.maxTokens}` : ""}t` : undefined,
        error,
      );
...
export async function renderPromptNode(
  ...
  const appendUser = (content: string | PromptImage, options: ContextExpansionOptions) =>
    appendUserMessage(messages, content, options);
  const appendAssistant = (content: string, options: ContextExpansionOptions) =>
