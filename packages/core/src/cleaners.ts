 ...
 */
export function arrayify<T>(a: T | T[], options?: { filterEmpty?: boolean }): T[] {
...
export function deleteUndefinedValues<T extends Record<string, any>>(o: T): T {
  ...
  if (typeof o === "object") for (const k in o) if (o[k] === undefined) delete o[k];
  return o;
}
...
export function encodeIDs(
  ...
  const decode = (text: string) => text?.replace(drx, (encoded) => ids[encoded]);
