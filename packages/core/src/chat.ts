...
function toChatCompletionImage(image: PromptImage): ChatCompletionContentPartImage {
...
async function runToolCall(
  ...
  for (const todo of todos) {
    ...
    } else if (typeof output === "object" && (output as ShellOutput).exitCode !== undefined) {
...
${fenceMD(content, " ")}
`;
    } else if (typeof output === "object" && (output as RunPromptResult).text) {
    ...
    if (tool.options?.intent) {
      ...
      if (!generator) throw new Error("tool generator not found, cannot validate intent");
  ...
  const repair = invalids
    .map((f) =>
      toStringList(
        ...
        f.validation.schemaError ? `error: ${f.validation.schemaError}` : undefined,
      ),
    )
    .join("\n\n");
  ...
  if (logprobs?.length) {
    logVerbose(
      toStringList(
        ...
        !isNaN(perplexity) ? `perplexity: ${renderWithPrecision(perplexity, 3)}` : undefined,
        !isNaN(uncertainty) ? `uncertainty: ${renderWithPrecision(uncertainty, 3)}` : undefined,
      ),
    );
    try {
...
function parseAssistantMessage(resp: ChatCompletionResponse): ChatCompletionAssistantMessageParam {
  ...
  if (options.fallbackTools && assistantContent && tools.length) {
    ...
    for (const toolCallFence of toolCallFences) {
      for (const toolCall of toolCallFence.content.split("\n")) {
        const { name, args } =
          /^(?<name>[\w\d]+):\s*(?<args>\{.*\})\s*$/i.exec(toolCall)?.groups || {};
        if (name) {
          resp.toolCalls.push({
            id: undefined,
            name,
  ...
  if (chatParticipants?.length) {
    ...
    for (const participant of chatParticipants) {
      ...
      try {
        ...
        // expand template
        const { errors, messages: participantMessages } = await renderPromptNode(
          options.model,
          node,
          {
