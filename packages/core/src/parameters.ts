...
export function promptParameterTypeToJSONSchema(
...
): JSONSchemaNumber | JSONSchemaString | JSONSchemaBoolean | JSONSchemaObject | JSONSchemaArray {
  ...
  if (typeof t === "string")
    ...
    typeof t === "object" &&
    ["number", "integer", "string", "boolean", "object"].includes((t as any).type)
  ) {
    const { required, ...rest } = t as any;
    return <JSONSchemaNumber | JSONSchemaString | JSONSchemaBoolean | JSONSchemaObject>{ ...rest };
  } else if (typeof t === "object") {
    const o = {
      type: "object",
      properties: Object.fromEntries(
        Object.entries(t).map(([k, v]) => [k, promptParameterTypeToJSONSchema(v, options)]),
      ),
...
export function promptParametersSchemaToJSONSchema(
  ...
  const res: Required<Pick<JSONSchemaObject, "type" | "properties" | "required">> = {
    type: "object",
    properties: {},
    required: [],
  };
  ...
  for (const [k, v] of Object.entries(parameters as PromptParametersSchema)) {
    ...
    res.properties[k] = t;
    if (t.type !== "object" && t.type !== "array" && required) res.required.push(k);
  }
