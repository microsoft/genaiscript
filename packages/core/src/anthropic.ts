...
const convertFinishReason = (stopReason: Anthropic.Message["stop_reason"]): ChatFinishReason => {
  switch (stopReason) {
    case "end_turn":
      return "stop";
    case "max_tokens":
...
const convertMessages = (
  ...
  for (let i = 0; i < messages.length; ++i) {
    ...
    if (last?.role !== msg.role) res.push(msg);
      ...
      if (typeof msg.content === "string") last.content.push({ type: "text", text: msg.content });
...
const convertSingleMessage = (
  ...
  } else if (role === "function") throw new NotSupportedError("function message not supported");
...
const convertToolResultMessage = (msg: ChatCompletionToolMessageParam): Anthropic.MessageParam => {
  return {
    role: "user",
    content: [
      deleteUndefinedValues({
...
const convertImageUrlBlock = (block: ChatCompletionContentPartImage): Anthropic.ImageBlockParam => {
  return {
    type: "image",
    source: {
      type: "base64",
...
const convertTools = (tools?: ChatCompletionTool[]): Anthropic.Messages.Tool[] | undefined => {
