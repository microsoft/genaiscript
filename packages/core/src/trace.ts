...
export class MarkdownTrace extends EventTarget implements OutputTrace {
  ...
  get tree() {
    if (!this._tree) this._tree = parseTraceTree(this.content, { parseItems: true });
    return this._tree;
  }
  ...
  chatProgress(progress: ChatCompletionsProgressReport) {
    ...
    if (responseChunk) this.dispatchEvent(new TraceChunkEvent(responseChunk, inner, progress));
  }
   ...
   * @param rows
   */
  table(rows: object[], options?: { headers?: ElementOrArray<string> }): void {
    if (!rows?.length) return;
    const md = dataToMarkdownTable(rows, options);
    this.appendContent(`\n\n${md}\n\n`);
  }

  startDetails(title: string, options?: { success?: boolean; expanded?: boolean }) {
...
`);
  ...
  itemValue(name: string, value: any, unit?: string) {
    ...
    if (typeof value === "function") this.item(`${name}: ${value.name || "anonymous"}`);
  ...
  async image(urlOrImage: BufferLike, caption: string): Promise<void> {
  ...
  get errors(): { message: string; error: SerializedError }[] {
    const traces = this._content.filter((c) => typeof c !== "string") as MarkdownTrace[];
          ...
          if (content && maxLength > 0) {
            ...
            for (const secret of Object.values(secrets))
              preview = preview.replaceAll(
                secret,
                secret.slice(0, 3) + "*".repeat(secret.length - 3),
              );
            this.detailsFenced(`<code>${file.filename}</code>: ${suffix}`, preview, "text");
          } else this.itemValue(`\`${file.filename}\``, toStringList(size, score));
        }
      } finally {
