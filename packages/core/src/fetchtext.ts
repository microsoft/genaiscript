...
export async function fetchText(
  ...
  if (isBinaryMimeType(mime?.mime)) {
    ...
    else if (convert === "text") content = await HTMLToText(content, { trace, cancellationToken });
    else if (convert === "tables") content = JSON.stringify(await HTMLTablesToJSON(content));
  }
  ok = true;
