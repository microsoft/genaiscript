...
export async function resolveBufferLike(
  ...
  if (typeof bufferLike === "string")
  ...
  else if (bufferLike instanceof Blob) return bufferTryFrom(await bufferLike.arrayBuffer());
  ...
  } else if (bufferLike instanceof ArrayBuffer) return bufferTryFrom(bufferLike);
  else if (bufferLike instanceof SharedArrayBuffer) return bufferTryFrom(bufferLike);
    ...
    return Buffer.from(await resolveFileBytes(bufferLike as WorkspaceFile, options));
  }
...
export async function resolveBufferLikeAndExt(
  ...
  } else if (typeof bufferLike === "string") return { bytes, ext: extname(bufferLike) };
