 ...
 */
export function srtVttRender(transcription) {
 ...
 */
export function parseTimestamps(transcription) {
    const ts = [];
 ...
 */
export function vttSrtParse(transcription) {
    if (!transcription)
        return [];
    const p = parse(transcription);
    return p.entries.map((e) => deleteEmptyValues({
        id: e.id,
        start: e.from,
        end: e.to,
        text: e.text,
