...
export function createChatTurnGenerationContext(
  ...
  const defImages = (
    files: ElementOrArray<string | WorkspaceFile | Buffer | Blob | ReadableStream>,
    ...
    if (Array.isArray(files)) {
      ...
      if (!defOptions?.tiled) sliced.forEach((file) => defImages(file, defOptions));
      else {
        appendChild(
          node,
          createImageNode(
  ...
  const ctx: ChatTurnGenerationContext & { node: PromptNode } = {
    ...
    def: (name, body, defOptions) => {
      ...
      if (body === undefined || body === null) {
      ...
      } else if (typeof body === "object" && (body as WorkspaceFile).filename) {
        ...
        if (endsWith.length && !endsWith.some((ext) => filename.endsWith(ext))) return undefined;
      ...
      } else if (typeof body === "object" && (body as ShellOutput).exitCode !== undefined) {
        appendChild(
          node,
          createDef(
            name,
      ...
      } else if (typeof body === "object" && (body as RunPromptResult).text) {
...
export function createChatGenerationContext(
  ...
  const defTool: (
    ...
    checkCancelled(cancellationToken);
    if (name === undefined || name === null) throw new Error("tool name is missing");
    dbg(`tool %s`, name);
    if (typeof name === "string") {
      if (typeof description !== "string") throw new Error("tool description is missing");
  ...
  const defAgent = (
    name: string,
    description: string,
    fn: (agentCtx: ChatGenerationContext, args: ChatFunctionArgs) => Promise<void>,
    ...
    defTool(
      ...
      async (args) => {
        ...
        const res = await ctx.runPrompt(
          async (_) => {
            if (typeof fn === "string") _.writeText(dedent(fn), { role: "system" });
  ...
  const defSchema = (name: string, schema: JSONSchema, defOptions?: DefSchemaOptions) => {
    checkCancelled(cancellationToken);
  ...
  const defChatParticipant = (
    ...
    checkCancelled(cancellationToken);
    if (generator) appendChild(node, createChatParticipant({ generator, options }));
  };
  ...
  const prompt = (
    ...
    const p: RunPromptResultPromiseWithOptions = new Promise<RunPromptResult>(
  ...
  const transcribe = async (
    ...
    try {
      ...
      const _cache = createCache<{ file: Blob } & TranscriptionOptions, TranscriptionResult>(
        cache === true ? TRANSCRIPTION_CACHE_NAME : typeof cache === "string" ? cache : undefined,
        { type: "fs" },
      );
      if (cache) {
  ...
  const runPrompt = async (
    ...
    try {
      ...
      if (typeof generator === "string") runCtx.node.children.push(createTextNode(generator));
      ...
      if (systemScripts.length)
        try {
          ...
          for (const systemId of systemScripts) {
            ...
            if (sysr.images?.length) throw new NotSupportedError("images");
            if (sysr.schemas) Object.assign(schemas, sysr.schemas);
            if (sysr.functions) tools.push(...sysr.functions);
            if (sysr.fileMerges?.length) fileMerges.push(...sysr.fileMerges);
            if (sysr.outputProcessors?.length) outputProcessors.push(...sysr.outputProcessors);
      ...
      if (!completer) throw new Error("model driver not found for " + info.model);
      checkCancelled(cancellationToken);

      const modelConcurrency =
        options.modelConcurrency?.[genOptions.model] ?? CHAT_REQUEST_PER_MODEL_CONCURRENT_LIMIT;
  ...
  // Helper function to setup model configuration and provider
  const setupImageModel = async (
    modelId: string | undefined,
    traceTitle: string,
    operationType: string,
  ) => {
    const imgTrace = trace?.startTraceDetails(traceTitle);
    const conn: ModelConnectionOptions = {
      model: modelId || IMAGE_GENERATION_MODEL_ID,
    };
    const { info, configuration } = await resolveModelConnectionInfo(conn, {
      trace: imgTrace,
    ...
    const statsChild = options.stats?.createChild(info.model, operationType);
    checkCancelled(cancellationToken);
    const { ok } = await runtimeHost.pullModel(configuration, {
      trace: imgTrace,
      cancellationToken,
  ...
  const processImageResults = async (
    imageResults: BufferLike[],
    imgTrace: any,
    imageOptions?: ImageGenerationOptions,
    labelPrefix: string = "image",
