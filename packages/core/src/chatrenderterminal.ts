...
function renderTrimmed(s: string, rows: number, width: number) {
  ...
  const res = trimmed.map((l, i) =>
    wrapColor(CONSOLE_COLOR_DEBUG, BOX_UP_AND_DOWN + ellipse(l, width) + "\n"),
  );
  return res;
}
  ...
  // If the content is an array, process each element based on its type.
    ...
    for (const c of content) {
      ...
      switch (c.type) {
        ...
        case "image_url":
          res.push(
            await renderImageToTerminal(dataUriToBuffer(c.image_url.url), {
              columns,
              rows,
              cancellationToken,
              modelId,
...
function renderToolCall(call: ChatCompletionMessageToolCall, options: { columns: number }): string {
