...
function renderJSONSchemaToFunctionParameters(
  ...
  if (!schema) return "";
    ...
    else if (single.type === "number" || single.type === "integer") return "number";
  }
  return "?";
}
...
export function JSONSchemaStringifyToTypeScript(
  ...
  function stringifyNode(node: JSONSchemaType): string {
    if (node === undefined) return "any";
      ...
      else if (n.type === "number" || n.type === "integer") return "number";
    }
    return "unknown";
  }
  ...
  function stringifyNodeDoc(node: JSONSchemaType): string {
    ...
    switch (n.type) {
      ...
      case "integer": {
        ...
        if (n.exclusiveMinimum !== undefined) doc.push(`exclusiveMinimum: ${n.exclusiveMinimum}`);
        if (n.exclusiveMaximum !== undefined) doc.push(`exclusiveMaximum : ${n.exclusiveMaximum}`);
...
export function validateFencesWithSchema(
  ...
  for (const fence of fences?.filter(
    ({ language, args }) => args?.schema && (language === "json" || language === "yaml"),
  )) {
    ...
    if (!fence.validation) {
      ...
      } else fence.validation = validateJSONWithSchema(data, schemaObj, options);
    }
...
export function toStrictJSONSchema(
  ...
  function visit(node: JSONSchemaType): void {
    ...
    switch (n.type) {
      ...
      case "object": {
        if (n.additionalProperties) throw new Error("additionalProperties: true not supported");
