...
export function createChatGenerationContext(options, trace, projectOptions) {
    ...
    const generateImage = async (prompt, imageOptions) => {
        ...
        if (!prompt && !hasInputImages)
            throw new Error("prompt or image(s) is required");
        ...
        try {
            ...
            if (inputImages && inputImages.length > 0) {
                for (let i = 0; i < inputImages.length; i++) {
                    ...
                    imageDataArray.push(imageData);
                    imgTrace?.itemValue(`input_image_${i + 1}`, typeof inputImages[i] === 'string' ? 'inline image' : inputImages[i].filename || 'inline image');
                }
            }
            const conn = {
