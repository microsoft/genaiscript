...
describe("Trace Memory Optimization", () => {
  ...
  test("Memory optimization verification", () => {
    ...
    const runTrace = false;
    const trace = runTrace ? (() => { traceCreated = true; return new MarkdownTrace({ cancellationToken }); })() : undefined;
    const outputTrace = (() => { outputTraceCreated = true; return new MarkdownTrace({ cancellationToken }); })();
