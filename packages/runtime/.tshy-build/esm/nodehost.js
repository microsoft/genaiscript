...
import { AZURE_AI_INFERENCE_TOKEN_SCOPES, AZURE_COGNITIVE_SERVICES_TOKEN_SCOPES, AZURE_MANAGEMENT_TOKEN_SCOPES, MODEL_PROVIDER_AZURE_AI_INFERENCE, MODEL_PROVIDER_AZURE_OPENAI, MODEL_PROVIDER_AZURE_SERVERLESS_MODELS, MODEL_PROVIDER_AZURE_SERVERLESS_OPENAI, NEGATIVE_GLOB_REGEX, SHELL_EXEC_TIMEOUT, LogEvent, McpClientManager, ResourceManager, NotSupportedError, assert, arrayify, ci, createAzureContentSafetyClient, createAzureTokenResolver, createNodePath, createWorkspaceFileSystem, defaultModelConfigurations, errorMessage, filterGitIgnore, genaiscriptDebug, logError, logVerbose, mergeHostConfigs, parseModelIdentifier, parseTokenFromEnv, PLimitPromiseQueue, providerFeatures, readHostConfig, resolveLanguageModel, shellQuote, setRuntimeHost, isAzureContentSafetyClientConfigured, } from "@genaiscript/core";
...
export class NodeHost extends EventTarget {
    pulledModels = [];
    _dotEnvPaths;
    _hostConfig = {};
    project;
    ...
    workspace = createWorkspaceFileSystem();
    containers = new DockerManager();
    _modelAliases = {
    ...
    _config;
    userInputQueue = new PLimitPromiseQueue(1);
    azureToken;
    ...
    constructor(dotEnvPaths) {
        ...
        this.azureToken = createAzureTokenResolver("Azure OpenAI", "AZURE_OPENAI_TOKEN_SCOPES", AZURE_COGNITIVE_SERVICES_TOKEN_SCOPES);
        this.azureAIInferenceToken = createAzureTokenResolver("Azure AI Inference", "AZURE_AI_INFERENCE_TOKEN_SCOPES", AZURE_COGNITIVE_SERVICES_TOKEN_SCOPES);
        this.azureAIServerlessToken = createAzureTokenResolver("Azure AI Serverless", "AZURE_SERVERLESS_OPENAI_TOKEN_SCOPES", AZURE_AI_INFERENCE_TOKEN_SCOPES);
        this.azureManagementToken = createAzureTokenResolver("Azure Management", "AZURE_MANAGEMENT_TOKEN_SCOPES", AZURE_MANAGEMENT_TOKEN_SCOPES);
        this.microsoftGraphToken = createAzureTokenResolver("Microsoft Graph", "MICROSOFT_GRAPH_TOKEN_SCOPES", ["https://graph.microsoft.com/.default"]);
    ...
    async getLanguageModelConfiguration(modelId, options) {
        ...
        if (askToken && tok && !tok.token) {
            if (tok.provider === MODEL_PROVIDER_AZURE_OPENAI ||
                ...
                if (!azureToken) {
                    ...
                    if (azureTokenError) {
                        logError(`${providerName} token not available for ${modelId}, ${tok.azureCredentialsType || "default"}`);
                ...
                if (!azureToken) {
                    if (azureTokenError) {
                        logError(`Azure AI Inference token not available for ${modelId}, ${tok.azureCredentialsType || "default"}`);
                        logVerbose(azureTokenError.message);
    ...
    async exec(containerId, command, args, options) {
        ...
        const { label, cwd, timeout = SHELL_EXEC_TIMEOUT, cancellationToken, stdin: input, ignoreError, env, isolateEnv, } = options || {};
