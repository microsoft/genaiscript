...
exports.markdownifyPdf = markdownifyPdf;
 ...
 */
async function markdownifyPdf(file, options) {
    const generator = (0, core_1.resolveChatGenerationContext)(options);
    const { label = `markdownify PDF`, model = "ocr", responseType = "markdown", instructions, ...rest } = options || {};
    ...
    for (let i = 0; i < pages.length; ++i) {
        ...
        const res = await generator.runPrompt(async (_) => {
            ...
            _.$ `You are an expert at converting PDFs to markdown.
                ...
                `;
            if (image)
                _.$ `- For images, generate a short alt-text description.`;
...
//# sourceMappingURL=markdownifypdf.js.map