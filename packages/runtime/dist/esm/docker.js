...
import { CORE_VERSION, DOCKER_CONTAINER_VOLUME, DOCKER_DEFAULT_IMAGE, DOCKER_VOLUMES_DIR, PLimitPromiseQueue, arrayify, dotGenaiscriptPath, ensureDir, errorMessage, genaiscriptDebug, generateId, hash, isQuiet, logError, logVerbose, resolveRuntimeHost, shellParse, shellQuote, } from "@genaiscript/core";
...
export class DockerManager {
    ...
    async containerName(options) {
        const { image = DOCKER_DEFAULT_IMAGE, persistent, name: userName, ports, postCreateCommands, env, networkEnabled, } = options;
    ...
    async internalStartContainer(options) {
        const { image = DOCKER_DEFAULT_IMAGE, trace, env = {}, networkEnabled, postCreateCommands, } = options;
