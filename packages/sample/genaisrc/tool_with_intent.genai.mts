...
defTool(
    ...
    async (args) => {
        const { content, location, sidenote } = args
        if (!sidenote)
        ...
        return `The weather in ${args.location} is sunny with a high of 75°F and a low of 55°F, and the package version is ${sidenote}.`
    }
)
