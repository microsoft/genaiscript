 ...
 */
export async function runScriptInternal(scriptId, files, options) {
    ...
    const { trace = new MarkdownTrace({ cancellationToken, dir: runDir }), runOutputTrace = new MarkdownTrace({ cancellationToken, dir: runDir }), infoCb, partialCb, } = options || {};
    ...
    else if (GENAI_ANY_REGEX.test(scriptId))
        toolFiles.push(scriptId);
    ...
    const outTraceFilename = options.runTrace === false || (isCI && !options.runTrace) || script.disableTrace
        ? undefined
        : await setupTraceWriting(trace, "trace", join(runDir, TRACE_FILENAME));
    const outputFilename = options.outputTrace === false || (isCI && !options.outputTrace)
        ? undefined
    ...
    for (let arg of files) {
        ...
        if (uriArg) {
            ...
            if (!resource)
                return fail(`resource ${arg} not found`, FILES_NOT_FOUND_ERROR_CODE);
        ...
        if (!ffs?.length && arg.includes("*")) {
            ...
            return fail(`no files matching ${arg} under ${process.cwd()} (all files might have been ignored)`, FILES_NOT_FOUND_ERROR_CODE);
        }
    ...
    runOutputTrace.addEventListener(TRACE_CHUNK, (ev) => {
        ...
        if (progress) {
            ...
            if (!isQuiet &&
                ...
                reasoningChunk !== "") {
                if (!reasoningOutput)
                    stderr.write(reasoningStartMarker);
    ...
    try {
        ...
        if (info.error) {
            ...
            return fail(info.error ?? "invalid model configuration", CONFIGURATION_ERROR_CODE, DOCS_CONFIGURATION_URL);
        }
    ...
    if (outAnnotations && result.annotations?.length) {
        if (isJSONLFilename(outAnnotations))
            ...
            await writeText(outAnnotations, CSV_REGEX.test(outAnnotations)
    ...
    if (outData && result.frames?.length) {
        if (isJSONLFilename(outData))
            await appendJSONL(outData, result.frames);
        else
            await writeText(outData, JSON.stringify(result.frames, null, 2));
    }
    ...
    if (result.schemas) {
        for (const [sname, schema] of Object.entries(result.schemas)) {
            await writeText(join(runDir, `${sname.toLocaleLowerCase()}.schema.ts`), JSONSchemaStringifyToTypeScript(schema, {
                typeName: capitalize(sname),
                export: true,
            }));
            await writeText(join(runDir, `${sname.toLocaleLowerCase()}.schema.json`), JSONSchemaStringify(schema));
        }
    }
    if (annotationf) {
        await writeText(annotationf, `severity, filename, start, end, message\n` +
            result.annotations
                .map(({ severity, filename, range, message }) => `${severity}, ${filename}, ${range[0][0]}, ${range[1][0]}, ${message} `)
                .join("\n"));
    }
    if (sariff)
        await writeText(sariff, await convertDiagnosticsToSARIF(script, result.annotations));
    ...
    for (const [filename, edits] of Object.entries(result.fileEdits || {})) {
        ...
        if (!isAbsolutePath)
            await writeText(join(runDir, CLI_RUN_FILES_FOLDER, rel), edits.after);
    }
    ...
    if (pullRequestComment && result.text) {
        ...
        if (ghInfo.repository &&
            ...
            await githubCreateIssueComment(script, ghInfo, result.text, typeof pullRequestComment === "string" ? pullRequestComment : script.id, { cancellationToken, stats });
        }
        else {
            ...
            if (adoInfo.collectionUri &&
                ...
                await azureDevOpsCreateIssueComment(script, adoInfo, prettifyMarkdown(result.text), typeof pullRequestComment === "string" ? pullRequestComment : script.id);
            }
            else
                logError("pull request comment: no pull request information found");
        }
    }
    if (pullRequestDescription && result.text) {
        ...
        if (ghInfo.repository &&
            ...
            await githubUpdatePullRequestDescription(script, ghInfo, prettifyMarkdown(result.text), typeof pullRequestDescription === "string" ? pullRequestDescription : script.id, { cancellationToken });
        }
        else {
            ...
            if (adoInfo.collectionUri &&
                ...
                await azureDevOpsUpdatePullRequestDescription(script, adoInfo, prettifyMarkdown(result.text), typeof pullRequestDescription === "string" ? pullRequestDescription : script.id);
            }
            else {
    ...
    if (result.status === "success")
        ...
        logError(`genaiscript: ${result.status}`);
    stats.log();
    if (outTraceFilename)
