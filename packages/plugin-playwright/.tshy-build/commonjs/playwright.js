...
const dbg = (0, core_1.genaiscriptDebug)("playwright");
/**
...
class BrowserManager {
    ...
    async launchBrowser(options) {
        const { browser = core_1.PLAYWRIGHT_DEFAULT_BROWSER, connectOverCDP, ...rest } = options || {};
        const launch = async () => {
    ...
    async browse(url, options) {
        ...
        dbg(`browsing ${(0, core_1.uriRedact)(url)}`);
        ...
        if (incognito || recordVideo) {
            ...
            if (recordVideo) {
                const dir = await (0, core_1.createVideoDir)();
