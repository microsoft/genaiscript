...
const genaiscriptSchema = Object.freeze({
  ...
  attributes: {
    ...
    svg: [["className", "octicon", /^markdown-./], "viewBox", "width", "height", /^aria-./],
    path: ["d", /^aria-./],
    span: [["className", /^logprobs$/], "style"],
    table: [["className", /^toplogprobs$/], "style"],
    details: [["className", /^reasoning$/]],
    code: [["className", /^language-./, /^aria-./, "math-inline", "math-display"]],
  },
});
...
const readmeSchema = Object.freeze({
  ...
  tagNames: genaiscriptSchema.tagNames.filter(
    (tag) => tag !== "img" && tag !== "video" && tag !== "audio" && tag !== "iframe",
  ),
});
...
export default function Markdown(props: {
  ...
  const { className, aiDisclaimer, filename, text, children, copySaveButtons, readme } = props;
      ...
      {isVisible && children ? (
        <ErrorBoundary fallback={<p>⚠️Something went wrong while rendering markdown.</p>}>
          <ReactMarkdown
            ...
            rehypePlugins={[
              rehypeRaw,
              [rehypeSanitize, readme ? readmeSchema : genaiscriptSchema],
              rehypeMathML,
              [rehypeHighlight, { ignoreMissing: true }],
            ]}
            remarkPlugins={[remarkMath, remarkGfm, [remarkAlert, { tagName: "blockquote" }]]}
          >
            {children}
          </ReactMarkdown>
          {copySaveButtons ? (
            <CopySaveButtons aiDisclaimer={aiDisclaimer} filename={filename} text={text}>
              {children}
            </CopySaveButtons>
          ) : null}
        </ErrorBoundary>
