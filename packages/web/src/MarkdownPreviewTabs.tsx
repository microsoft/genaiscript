...
export default function MarkdownWithPreviewTabs(props: {
  ...
  const md = useMemo(
    () =>
      cleanMarkdown(convertThinkToMarkdown(convertAnnotationsToMarkdown(renderText || cleaned))),
    [cleaned, renderText],
  );
  return (
    <>
      ...
      <vscode-tab-panel>
        {text ? (
          <Markdown aiDisclaimer={true} copySaveButtons={true} filename={filename} text={cleaned}>
