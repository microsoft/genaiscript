...
function JSONSchemaString(props: {
  ...
  if (field.uiType === "textarea") return <JSONSchemaStringTextArea {...props} />;
...
function JSONSchemaStringTextArea(props: {
  ...
  const { field, required, value, onChange } = props;
  const rows = (s: string | undefined) => Math.max(3, s?.split("\n").length ?? 0);
  return (
    <vscode-textarea
...
function JSONSchemaNumber(props: {
  ...
  const [valueText, setValueText] = useState(isNaN(value) ? "" : String(value));

  useEffect(() => {
    const v = type === "number" ? parseFloat(valueText) : parseInt(valueText);
    ...
    <vscode-checkbox-group>
      {Object.entries(properties).map(([fieldName, field]) => (
        <vscode-checkbox
          ...
          onChange={(e) =>
            handleFieldChange(fieldPrefix + fieldName, (e.target as HTMLInputElement).checked)
          }
        />
      ))}
    </vscode-checkbox-group>
...
export function JSONSchemaObjectForm(props: {
  ...
  const properties: Record<string, JSONSchemaSimpleType> = schema.properties ?? ({} as any);
...
function JSONSchemaObjectPropertiesForm(props: {
  ...
  const fieldElements = properties.map(([fieldName, field]) => (
    <vscode-form-group key={fieldPrefix + fieldName}>
      <vscode-label>{fieldDisplayName(fieldPrefix, fieldName, field)}</vscode-label>
      <JSONSchemaSimpleTypeFormField
        field={field}
        value={value[fieldPrefix + fieldName]}
        required={schema.required?.includes(fieldName)}
        onChange={(value) => handleFieldChange(fieldPrefix + fieldName, value)}
      />
      {field?.description && (
        <vscode-form-helper>
