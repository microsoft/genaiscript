...
import { useApi, useScripts, useEnv, useScript, useModels, ApiProvider } from "./ApiContext";
      ...
      <vscode-form-group>
        ...
        >
          {importedFiles.map((file) => (
            <vscode-option key={file.path} value={file.path} selected={file.selected}>
...
function RefreshButton() {
  ...
  return <ActionButton name="refresh" label="reload script list" onClick={refresh} />;
}
...
function RunButtonOptions() {
  ...
  const scriptParameters = inputSchema.properties["script"] as JSONSchemaObject;
  const runOptions: [string, JSONSchemaBoolean][] = scriptParameters?.properties
    ? Object.entries(scriptParameters.properties as Record<string, JSONSchemaSimpleType>)
        .filter(([, f]) => f.type === "boolean" && f.uiType === "runOption")
...
function PromptParametersFields() {
  ...
  const scriptParameters = inputSchema.properties["script"] as JSONSchemaObject;
  const systemParameters = Object.entries(inputSchema.properties).filter(([k]) => k !== "script");
  return (
    <>
      {scriptParameters && (
      ...
      {!!systemParameters.length && (
        <vscode-collapsible className="collapsible" title="System Parameters">
...
function ModelConfigurationTabPanel() {
  ...
  const { providers } = env || {};
  const models =
    providers?.flatMap((p) => p.models?.map((m) => `${p.provider}:${m.id}`) || []) || [];
  ...
  const schema: JSONSchemaObject = {
    ...
    properties: {
      ...
      logprobs: {
        type: "boolean",
        description: "Enable reporting log probabilities for each token",
        default: false,
      },
      topLogprobs: {
        type: "integer",
        description: "Enables reporting log probabilities for alternate tokens",
        minimum: 0,
        maximum: 5,
        default: 0,
      },
...
function ChatCompletationTabPanel() {
  ...
  const [userContent, setUserContent] = useState<string>("write a poem using emojis");
  const [response, setResponse] = useState<ChatCompletion | { error?: string }>(undefined);
              ...
              <vscode-form-group>
                ...
                <vscode-tabs>
                  <MarkdownPreviewTabs
                    ...
                    renderText={
                      (response as ChatCompletion)?.choices?.length
                        ? (response as ChatCompletion)?.choices
    ...
    if (!providers) ref.current.data = [];
      ...
      const data: TreeItem[] = PROVIDERS.filter(
        ...
        .map(
          (r) =>
            ({
              icons: r.error ? errorIcons : r.missing ? missingIcons : icons,
              label: r.provider,
...
function ClientReadyStateLabel() {
  ...
  return (
    <vscode-label title={`server connection status: ${readyState}`}>{readyState}</vscode-label>
  );
}
