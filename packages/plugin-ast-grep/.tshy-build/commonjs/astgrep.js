...
exports.astGrep = astGrep;
// Copyright (c) Microsoft Corporation.
// Licensed under the MIT License.
const core_1 = require("@genaiscript/core");
const path_1 = require("path");
const dbg = (0, core_1.genaiscriptDebug)("astgrep");
const dbgLang = dbg.extend("lang");
class SgChangeSetImpl {
    pending = {};
    toString() {
    ...
    replace(node, text) {
        ...
        if (rootEdits) {
            if (rootEdits.root !== root) {
                throw new Error(`node ${node} belongs to a different root ${root} than the pending edits ${rootEdits.root}`);
            }
        }
        else
            rootEdits = this.pending[root.filename()] = { root, edits: [] };
 ...
 */
function astGrepCreateChangeSet() {
 ...
 */
async function astGrepFindFiles(lang, glob, matcher, options) {
    ...
    const diffFiles = (0, core_1.diffResolve)(diff);
    const runtimeHost = (0, core_1.resolveRuntimeHost)();
    dbg(`search %O`, matcher);
    if (diffFiles?.length)
        dbg(`diff files: ${diffFiles.length}`);
    ...
    const p = new Promise(async (resolve, reject) => {
        ...
        n = await findInFiles(sglang, {
            paths,
            matcher: typeof matcher === "string" ? { rule: { pattern: matcher } } : matcher,
        }, (err, nodes) => {
            if (err) {
            ...
            if (cancellationToken?.isCancellationRequested) {
                reject(new core_1.CancelError("cancelled"));
            }
            if (++i === n) {
    ...
    if (diffFiles?.length) {
        matches = matches.filter((m) => {
            ...
            const { chunk } = (0, core_1.diffFindChunk)(m.getRoot().filename(), range, diffFiles) || {};
            if (chunk) {
                dbg(`diff overlap at (${range[0]},${range[1]}) x (${chunk.newStart},${chunk.newStart + chunk.newLines})`);
            }
            return chunk;
        });
 ...
 */
async function astGrepParse(file, options) {
...
async function resolveLang(lang, filename) {
    const norm = (l) => l.toLowerCase().replace(/^\./, "");
    ...
    if (typeof lang === "string") {
        ...
        if (builtin)
            ...
            return loadDynamicLanguage(lang);
    }
    if (!filename) {
    ...
    if (filename) {
        const ext = norm((0, path_1.extname)(filename));
        ...
        if (dynamic)
            return loadDynamicLanguage(dynamic);
...
const loadedDynamicLanguages = new Set();
async function loadDynamicLanguage(langName) {
    if (!loadedDynamicLanguages.has(langName)) {
        ...
        try {
            ...
            dbgLang(`error loading language ${langName}: ${(0, core_1.errorMessage)(err)}`);
            throw Error(`@ast-grep/lang-${langName} package failed to load, please install it using 'npm install -D @ast-grep/lang-${langName}'`);
        }
    }
    return langName;
}
async function astGrep(options) {
    ...
    return Object.freeze({
        ...
        search: (lang, glob, matcher, sgOptions) => astGrepFindFiles(lang, glob, matcher, {
            ...(sgOptions || {}),
            cancellationToken,
        }),
        parse: (file, sgOptions) => astGrepParse(file, {
            ...(sgOptions || {}),
            cancellationToken,
        }),
    });
