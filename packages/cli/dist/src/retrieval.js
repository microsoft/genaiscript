...
import { YAMLStringify, expandFiles, fuzzSearch, normalizeFloat, normalizeInt, vectorIndex, vectorSearch, } from "@genaiscript/core";
/**
 ...
 */
export async function retrievalIndex(indexName, filesGlobs, options) {
    const { excludedFiles, embeddingsModel, ignoreGitIgnore, database } = options || {};
    const files = (await expandFiles(filesGlobs, {
        excludedFiles,
 ...
 */
export async function retrievalSearch(q, filesGlobs, options) {
    // Destructure options with default values
    const { excludedFiles, name: indexName, topK, minScore, embeddingsModel, ignoreGitIgnore, } = options || {};
...
export async function retrievalFuzz(q, filesGlobs, options) {
    ...
    const res = await fuzzSearch(q, files.map((filename) => ({ filename })), { topK: normalizeInt(topK), minScore: normalizeFloat(minScore) });
    // Output the results in YAML format for readability
    console.log(YAMLStringify(res.map(({ filename, score }) => ({ filename, score }))));
}
