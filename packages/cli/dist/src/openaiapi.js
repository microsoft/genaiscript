...
import { LARGE_MODEL_ID, errorMessage, generateId, logError, logVerbose, resolveLanguageModel, resolveLanguageModelConfigurations, resolveModelConnectionInfo, } from "@genaiscript/core";
...
function endError(res, statusCode, message) {
 ...
 */
export async function openaiApiChatCompletions(req, res, options) {
    const { trace, cancellationToken } = options || {};
    try {
        logVerbose(`chat/completions: post`);
        const body = await readRequestBody(req);
        if (!body)
            return endError(res, 400, `Invalid request body`);
        if (body.stream)
            return endError(res, 400, `Streaming not supported`);
        const connection = await resolveModelConnectionInfo({ model: body.model }, { token: true, trace, defaultModel: LARGE_MODEL_ID });
        ...
        const resp = await completer(body, connection.configuration, { cancellationToken, inner: false }, trace);
 ...
 */
export async function openaiApiModels(req, res, options) {
    ...
    try {
        ...
        const resp = {
            ...
            data: providers
                ...
                .flatMap(({ provider, models }) => models.map(({ id }) => ({
                id: `${provider}:${id}`,
                owned_by: provider,
            }))),
        };
