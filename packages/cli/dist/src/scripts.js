...
import { CONSOLE_COLOR_DEBUG, copyPrompt, createScript as coreCreateScript, dedent, deleteEmptyValues, filterScripts, fixGitHubCopilotInstructions, fixPromptDefinitions, JSONSchemaToFunctionParameters, wrapColor, } from "@genaiscript/core";
 ...
 */
export async function listScripts(ids, options) {
    const { json } = options || {};
    ...
    if (!json)
        console.log(scripts.map(({ id, filename }) => `${id} - ${filename}`).join("\n"));
    else {
        console.log(JSON.stringify(scripts.map(({ id, title, group, filename, inputSchema, isSystem }) => deleteEmptyValues({
            id,
            title,
            group,
            filename,
...
export async function scriptInfo(scriptId) {
    ...
    if (accept !== "none")
        sigArguments.unshift(`files: ${accept || "*"}`);
    const sig = sigArguments.join(",\n  ");
    const secondary = (s) => wrapColor(CONSOLE_COLOR_DEBUG, s);
    console.log(secondary(dedent `/** 
        * ${title || ""} 
 ...
 */
export async function createScript(name, options) {
