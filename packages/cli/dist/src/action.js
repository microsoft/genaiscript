...
import { CORE_VERSION, GENAI_ANY_REGEX, GENAI_SRC, GitHubClient, MODEL_PROVIDERS, MODEL_PROVIDER_AZURE_OPENAI, MODEL_PROVIDER_GITHUB, MODEL_PROVIDER_OPENAI, MODEL_PROVIDER_AZURE_AI_INFERENCE, YAMLStringify, YAMLTryParse, createScript as coreCreateScript, dedent, deleteEmptyValues, deleteUndefinedValues, genaiscriptDebug, isCI, logInfo, logVerbose, nodeTryReadPackage, templateIdFromFileName, titleize, tryReadText, tryStat, writeText, resolveRuntimeHost, } from "@genaiscript/core";
 ...
 */
export async function actionConfigure(scriptId, options) {
    options = options || {};
    const { owner, repo } = (await github.info()) || {};
    if (!owner || !repo)
        throw new Error("GitHub repository information not found.");
    const { force, out = resolve("."), provider, pullRequestComment, pullRequestDescription, pullRequestReviews, interactive, } = options;
    ...
    let script = prj.scripts.find((t) => t.id === scriptId ||
    ...
    ].includes(id)).filter(({ env }) => env);
    const inputs = deleteUndefinedValues({
        ...Object.fromEntries(Object.entries(scriptSchema.properties).map(([key, value]) => {
            return [
                snakeCase(key),
                {
        ...
        github_issue: issue || pullRequest
            ? {
    ...
    let outputs = deleteUndefinedValues({
        text: {
    ...
    if (action && !force) {
        ...
        await writeFile("action.yml", YAMLStringify(deleteEmptyValues({
            name: titleize(repo),
            author: pkg?.author,
