...
import { RUNS_DIR_NAME, SERVER_PORT, TRACE_FILENAME, dotGenaiscriptPath, groupBy, resolveRuntimeHost, } from "@genaiscript/core";
/**
...
export async function collectRuns(options) {
    ...
    if (runsState?.type !== "directory")
        return [];
    const scripts = (await readdir(runsDir, {
        withFileTypes: true,
    }))
    ...
    for (const sid of scripts) {
        ...
        if ((await runtimeHost.statFile(sdir))?.type !== "directory")
            continue;
        const reports = (await readdir(sdir, {
            withFileTypes: true,
        })).filter((d) => d.isDirectory());
 ...
 */
export function resolveRunDir(scriptId, runId) {
