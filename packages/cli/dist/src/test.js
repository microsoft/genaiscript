...
function createEnv() {
    ...
    return {
        ...
        PROMPTFOO_DISABLE_REDTEAM_REMOTE_GENERATION: env.PROMPTFOO_DISABLE_REDTEAM_REMOTE_GENERATION ?? "true",
    };
}
/**
...
async function apiRunPromptScriptTests(ids, options) {
    ...
    for (const script of scripts) {
        ...
        if (testModels.length)
            dbgConfig(`test models: %o`, testModels);
    ...
    if (outSummary) {
        ...
        await appendFile(outSummary, [headersToMarkdownTableHead(headers), headersToMarkdownTableSeparator(headers)].join(""));
    }
    const results = [];
    try {
        for (const config of configurations) {
            ...
            logVerbose(BOX_UP_AND_RIGHT +
                BOX_RIGHT +
                toStringList(prettyDuration(elapsed), `${results.filter((r) => !r.ok).length} failed`, `${results.filter((r) => r.ok).length} success`, prettyTokens(stats.total, "both"), prettyTokens(stats.prompt, "prompt"), prettyTokens(stats.completion, "completion")));
...
`);
    ...
    for (const config of configurations) {
        ...
        logInfo(`test ${script.id} (${results.length + 1}/${configurations.length}) - ${configuration}`);
        const testStart = new Date();
        const outJson = configuration.replace(/\.yaml$/, ".res.json");
        const cmd = "npx";
        const args = ["--yes", `promptfoo@${promptFooVersion}`];
        if (redteam)
            args.push("redteam", "run", "--force");
        else
            args.push("eval", "--no-progress-bar");
