...
import { SUCCESS_ERROR_CODE, UNRECOVERABLE_ERROR_CODES, USER_CANCELLED_ERROR_CODE, ensureDotGenaiscriptPath, genaiscriptDebug, normalizeInt, createCancellationController, } from "@genaiscript/core";
 ...
 */
export async function runScriptWithExitCode(scriptId, files, options) {
    dbg(`run %s`, scriptId);
    ...
    for (let r = 0; r < runRetry; ++r) {
        ...
        if (exitCode === SUCCESS_ERROR_CODE || UNRECOVERABLE_ERROR_CODES.includes(exitCode))
            break;
        const delayMs = 2000 * Math.pow(2, r);
        if (runRetry > 1) {
            console.error(`error: run failed with ${exitCode}, retry #${r + 1}/${runRetry} in ${delayMs}ms`);
            await delay(delayMs);
        }
    }
