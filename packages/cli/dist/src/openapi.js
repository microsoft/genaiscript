...
import { CORE_VERSION, OPENAPI_SERVER_PORT, deleteUndefinedValues, ensureDotGenaiscriptPath, ensureHeadSlash, errorMessage, genaiscriptDebug, logError, logVerbose, logWarn, nodeTryReadPackage, toStrictJSONSchema, trimTrailingSlash, } from "@genaiscript/core";
...
export async function startOpenAPIServer(options) {
    ...
    const startServer = async () => {
        ...
        const { name, description = "GenAIScript OpenAPI Server", version = "0.0.0", author, license, homepage, displayName, } = (await nodeTryReadPackage()) || {};
        ...
        for (const tool of tools) {
            ...
            if (!group)
                logWarn(`${id}: operation must have a group`);
            ...
            const handler = async (request) => {
                ...
                const allParams = { ...(request.query || {}), ...bodyRest };
                dbgHandlers(`params: %O`, allParams);
                // Extract model parameters from HTTP request
                const { model, smallModel, visionModel, embeddingsModel, modelAlias, provider, temperature, reasoningEffort, topP, maxTokens, maxToolCalls, seed, toolChoice, ...vars } = allParams;
