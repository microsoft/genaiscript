...
import { LARGE_MODEL_ID, CORE_VERSION, YAMLStringify, deleteUndefinedValues, resolveLanguageModelConfigurations, resolveModelAlias, resolveModelConnectionInfo, resolveRuntimeHost, EMBEDDINGS_MODEL_ID, } from "@genaiscript/core";
 ...
 */
export async function envInfo(provider, options) {
 ...
 */
async function resolveScriptsConnectionInfo(scripts, options) {
    const runtimeHost = resolveRuntimeHost();
    const models = {};
    ...
    const res = await Promise.all(Object.values(models).map((conn) => resolveModelConnectionInfo(conn, {
        ...(options || {}),
 ...
 * @param options - Configuration options, including whether to show tokens.
 */
export async function scriptModelInfo(script, options) {
    const prj = await buildProject();
    const templates = prj.scripts.filter((t) => !script || t.id === script || resolve(t.filename) === resolve(script));
 ...
 */
export async function modelAliasesInfo(options) {
    const { check } = options || {};
    const runtimeHost = resolveRuntimeHost();
    const res = Object.fromEntries(Object.entries(runtimeHost.modelAliases).map(([k, v]) => [
        k,
        {
            ...v,
...
`,
            ...
            config.inference = inference?.error?.message || inference?.status || "error";
        }
    }
    console.log(YAMLStringify(res));
}
 ...
 */
export async function modelList(provider, options) {
    const runtimeHost = resolveRuntimeHost();
    ...
    if (options?.format === "json")
        ...
        console.log(YAMLStringify(deleteUndefinedValues(Object.fromEntries(providers.map((p) => [p.provider, p.error || p.models])))));
    }
}
