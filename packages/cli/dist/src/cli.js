...
import { CORE_VERSION, DEBUG_SCRIPT_CATEGORY, GITHUB_REPO, MODEL_PROVIDERS, OPENAI_MAX_RETRY_COUNT, OPENAI_MAX_RETRY_DELAY, OPENAI_RETRY_DEFAULT_DEFAULT, OPENAI_MAX_RETRY_AFTER_DEFAULT, RUNTIME_ERROR_CODE, SERVER_PORT, TOOL_ID, TOOL_NAME, UNHANDLED_ERROR_CODE, GitClient, errorMessage, genaiscriptDebug, isQuiet, isRequestError, logPerformance, logVerbose, semverSatisfies, serializeError, setConsoleColors, setQuiet, } from "@genaiscript/core";
...
export async function cli() {
    ...
    addPullRequestOptions(run)
        ...
        .option("--retry-delay <number>", "minimum delay between retries", String(OPENAI_RETRY_DEFAULT_DEFAULT))
        .option("--max-delay <number>", "maximum delay between retries", String(OPENAI_MAX_RETRY_DELAY))
        .option("--max-retry-after <number>", "maximum retry-after delay in milliseconds before giving up", String(OPENAI_MAX_RETRY_AFTER_DEFAULT))
