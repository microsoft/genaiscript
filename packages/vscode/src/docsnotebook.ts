...
const NOTEBOOK_LANG_ABBREVS = new Map(
  Array.from(NOTEBOOK_LANG_IDS.keys()).map((k) => [NOTEBOOK_LANG_IDS.get(k), k]),
);
const NOTEBOOK_MARKERS: Record<string, { startMarker: string; endMarker: string }> = {
  markdown: {
...
function activateNotebookExecutor(state: ExtensionState) {
  ...
  controller.executeHandler = async (cells, notebook) => {
    ...
    const { genaiscript: frontmatter = {} } =
      frontmatterTryParse(frontMatterText)?.value ?? YAMLTryParse(frontMatterText) ?? {};
    const { files, vars, ...runOptions }: NotebookFrontMatter = frontmatter || {};
    ...
    for (const cell of cells) {
      ...
      const syntax = mdx ? NOTEBOOK_MARKERS.mdx : NOTEBOOK_MARKERS.markdown;
      ...
      try {
        ...
        const {
          system = arrayify(meta.system)?.length > 0 ? undefined : "false",
          user,
          assistant,
        ...
        if (error) chat += details(`${EMOJI_FAIL} error`, errorMessage(error));
        chat = "\n\n" + syntax.startMarker + "\n" + chat + "\n" + syntax.endMarker + "\n\n";
        ...
        await execution.replaceOutput(
          [
            new vscode.NotebookCellOutput([
              vscode.NotebookCellOutputItem.text(chat, MARKDOWN_MIME_TYPE),
            ]),
            new vscode.NotebookCellOutput([
        ...
        await execution.replaceOutput([
          new vscode.NotebookCellOutput([vscode.NotebookCellOutputItem.error(e)]),
        ]);
      ...
      (data) =>
        <vscode.NotebookCellData>{
          ...
          outputs: data.output
            ? [
                new vscode.NotebookCellOutput([
                  vscode.NotebookCellOutputItem.text(data.output, MARKDOWN_MIME_TYPE),
                ]),
              ]
            : [],
          value: data.content,
      ...
      NOTEBOOK_TYPE,
      {
        ...
        serializeNotebook: function (data: vscode.NotebookData): Uint8Array {
          ...
          for (let i = 0; i < cells.length; i++) {
            const cell = cells[i];
            if (i === 0) result += cell.metadata?.leadingWhitespace ?? "";
            ...
            if (cell.kind === vscode.NotebookCellKind.Code) {
              if (cell.languageId === "yaml" && i === 0) {
                ...
                const languageAbbrev =
                  NOTEBOOK_LANG_ABBREVS.get(cell.languageId) ?? cell.languageId;
                const codePrefix = "```" + languageAbbrev + (options ? ` ${options}` : "") + "\n";
                result += codePrefix + cell.value + "\n```";
                const output = cell.outputs?.[0]?.items?.[0];
                if (output && output.mime === MARKDOWN_MIME_TYPE) {
                  const data = output.data;
                  result += decoder.decode(new Uint8Array(data));
                }
              }
            } else {
              result += cell.value;
  ...
  while (i < lines.length) {
    ...
    if (codeBlockMatch) {
      ...
      while (cs < lines.length) {
        ...
        if (/(<!--|\{\/\*)\s+genaiscript output start\s+(-->|\*\/\})/.test(lines[cs])) cf = true;
        break;
      }
      if (cf) {
        let ce = cs + 1;
        ...
        while (ce < lines.length) {
          if (/(<!--|\{\/\*)\s+genaiscript output end\s+(-->|\*\/\})/.test(lines[ce])) {
            cf = true;
            break;
          }
          ce++;
