    ...
    subscriptions.push(
      vscode.window.onDidCloseTerminal(async (e) => {
        if (e === this._terminal) {
          ...
          if (this._terminalStartAttempts > VSCODE_SERVER_MAX_RETRIES) {
            logInfo("server start attempts exceeded, trying out new port");
  ...
  private async startClient(): Promise<VsCodeClient> {
    ...
    client.addEventListener(RECONNECT, () => {
      ...
      if (client.connectedOnce) {
        const canReconnect =
          client.pending && this._terminalStartAttempts < VSCODE_SERVER_MAX_RETRIES;
        this.closeTerminal();
        if (canReconnect) {
