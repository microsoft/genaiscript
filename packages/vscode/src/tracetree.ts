...
class TraceTreeDataProvider implements vscode.TreeDataProvider<TraceNode> {
  ...
  getTreeItem(element: TraceNode): vscode.TreeItem | Thenable<vscode.TreeItem> {
    if (typeof element === "string") {
      ...
      if (element.type === "details") {
        ...
        item.collapsibleState =
          ...
          (element.content.filter((s) => typeof s !== "string").length > 0
            ? element.open
        ...
        if (typeof element.content[0] === "string" && element.content[0]) {
          const tooltip = new vscode.MarkdownString(element.content[0] as string, true);
          tooltip.isTrusted = false; // LLM, user generated
          item.tooltip = tooltip;
        }
