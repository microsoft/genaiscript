...
class PromptTreeDataProvider implements vscode.TreeDataProvider<PromptTreeNode> {
  ...
  async getTreeItem(element: PromptTreeNode): Promise<vscode.TreeItem> {
    if (typeof element === "string") {
      ...
      item.description = generating ? `${progress?.tokensSoFar || 0} tokens` : id;
      item.command = {
...
`,
      ...
      else if (element.unlisted) item.iconPath = new vscode.ThemeIcon("code");
      else if (!element.filename) item.iconPath = new vscode.ThemeIcon("lightbulb");
      else item.iconPath = new vscode.ThemeIcon("lightbulb-autofix");
      return item;
    }
  }

  async getChildren(element?: PromptTreeNode | undefined): Promise<PromptTreeNode[]> {
  ...
  private _onDidChangeTreeData: vscode.EventEmitter<void | PromptTreeNode | PromptTreeNode[]> =
    new vscode.EventEmitter<void | PromptTreeNode | PromptTreeNode[]>();
  readonly onDidChangeTreeData: vscode.Event<void | PromptTreeNode | PromptTreeNode[]> =
