...
for (const rule of Object.values(report.rules)) {
  ...
  const ruleInstances = projects.flatMap((p) => p.ruleInstances.filter((r) => r.ruleId === id));
        ...
        _.def(
          "ERROR",
          {
            ...
            content: `// incidentId: ${incidentId}
              ${location.line ? `[${location.line}] ${location.snippet}` : location.snippet}`,
          },
          { lineNumbers: false },
        );
      }
  ...
  for (const group of data) {
    ...
    for (const incidentId of incidents) {
      if (ruleInstances.find((r) => r.incidentId.toLowerCase() === incidentId.toLowerCase())) {
