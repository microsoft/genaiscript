...
<LinkCard title="z3" description="Solves a SMTLIB2 problem using the Z3 constraint solver. Send problems one at a time. If the constraint system is in another format, convert the problem to SMTLIB2 format before calling this tool." href="/genaiscript/reference/scripts/system#systemz3" />
