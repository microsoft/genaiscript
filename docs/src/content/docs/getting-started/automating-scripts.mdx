...
```sh wrap "--apply-edits"
genaiscript run <script> <...files> --apply-edits
```
...
```yaml
- run: genaiscript run <script> <...files> --out genairesults
  env:
      # variables
...
- run: genaiscript run ... --out-trace $GITHUB_STEP_SUMMARY
```
...
- run: genaiscript run --pull-request-description
```
...
- run: genaiscript run --pull-request-comment
  env: ...
```
...
- run: genaiscript run --pull-request-reviews
  env: ...
```
