...
for different models. For example, to enable the `phi3`
model running locally, while keeping the default `openai` model connection information.

```txt title=".env"
PHI3_API_BASE=http://localhost:...
```

### VSCode Copilot

Visual Studio Code **Insiders** provides an **experimental 
support** to use your Copilot subscription to access a LLM inside of Visual Studio Code.
