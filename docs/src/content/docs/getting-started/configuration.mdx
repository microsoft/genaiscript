...
This is not a LLM provider, but a content search provider. 
It allows you to search for content in your [Azure AI Search](https://learn.microsoft.com/en-us/azure/search/search-what-is-azure-search) service.

```js "azure_search:"
const index = await retreival.index("animals", { type: "azure_ai_search" })
