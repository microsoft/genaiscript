...
sidebar:
  order: 10
description: Découvrez comment générer automatiquement un texte alternatif
  descriptif pour les images en utilisant le modèle OpenAI Vision afin
  d'améliorer l'accessibilité et le référencement SEO.
...
Pour commencer, nous supposons que le script s'exécute sur un seul fichier image et nous utilisons [defImage](/genaiscript/reference/scripts/images) pour l'ajouter au contexte de la requête.
...
Enfin, nous utilisons [defFileOutput](/genaiscript/reference/scripts/file-output) pour définir une destination de sortie de fichier.
...
Avec la commande [run](/genaiscript/reference/cli/run), nous pouvons appliquer le script à chaque image dans la documentation.
