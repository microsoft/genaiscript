...
**What it does:**\
Le `script` function defines key metadata for your GenAIScript script, including a unique name, a clear description, and a file pattern to operate on.
...
**What it does:**\
Le `log()` function sends output to the console or GenAIScript logs. Logging early lets you know the script has started and is especially useful when diagnosing at which step things may be failing.
...
* Utilisez `log()` generously to illuminate your script’s behavior.
