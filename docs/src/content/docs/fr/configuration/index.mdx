...
Vous pouvez également utiliser les [alias de modèle](/genaiscript/reference/scripts/model-aliases) `small`, `large`, `vision` pour utiliser les modèles petits, grands et activés pour la vision configurés par défaut.
...
Les alias de modèles peuvent aussi être remplacés via la [commande cli run](/genaiscript/reference/cli/run),
...
* en spécifiant l'emplacement du fichier `.env` dans un [fichier de configuration](/genaiscript/reference/configuration-files).
...
La commande [configure](/genaiscript/reference/cli/configure) est une commande interactive pour configurer et valider les connexions LLM.
...
Vous pouvez vérifier votre configuration en exécutant la commande `genaiscript info env` [command](/genaiscript/reference/cli).
