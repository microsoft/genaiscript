...
The tool support is implemented in [system.tool\_calls](/genaiscript/reference/scripts/system#systemtool_calls)
...
* ajoutez la `fallbackTools` option to the script
...
Le `description` intent is a special value that gets expanded to the tool description.
