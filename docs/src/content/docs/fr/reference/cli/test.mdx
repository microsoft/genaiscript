...
Vous pouvez choisir les modèles à utiliser pour les tests avec l’option `--models` :
...
:::note
Cette fonctionnalité nécessite d’ajouter `@genaiscript/api` dans les dépendances de votre `package.json`.
:::

## visionneuse de résultats

Exécutez la commande `test view` pour lancer la visionneuse des résultats de test :
