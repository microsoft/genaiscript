...
sidebar:
  order: 1
description: Guide complet pour utiliser l’interface en ligne de commande (CLI)
  GenAIScript afin d’automatiser des tâches avec des scripts d’IA dans des
...
La CLI chargera les [secrets](/genaiscript/getting-started/configuration) depuis les variables d’environnement ou un fichier `./.env`.
...
[Exécute un script](/genaiscript/reference/cli/run) sur un fichier
...
La CLI peut être importée et [utilisée comme une API dans votre application Node.JS](/genaiscript/reference/api).
