...
* exécuter un script "directement". Ce scénario est utile lors du débogage d’un script ou pour un script qui ne nécessite pas de fichiers d’entrée.
...
* faites un clic droit dans l’éditeur et sélectionnez **Exécuter GenAIScript** dans le menu contextuel
* ou cliquez sur l’icône **Exécuter GenAIScript** dans le coin supérieur droit de l’éditeur
...
Ce mode est utile lors du développement de scripts ou pour des scripts qui ne nécessitent pas de fichiers d'entrée.

## Exécution des scripts à partir de fichiers ou dossiers d’entrée

Ce mode permet d’exécuter des scripts sur n’importe quelle combinaison de fichiers et de dossiers, qui alimentera `env.files`.
...
Cette valeur sera `undefined` si vous exécutez votre script depuis la ligne de commande, vous devez donc gérer ce cas dans votre script.

## Règles .gitignore

GenAIScript tente de respecter les **règles `.gitignore` de premier niveau dans l’espace de travail du projet** (il ignore les fichiers .gitignore imbriqués). Cela signifie que si vous avez un fichier `.gitignore` dans votre projet, GenAIScript ignorera tous les fichiers ou dossiers ignorés par Git.
...
Traduction réalisée avec l’aide d’une IA. Veuillez vérifier la précision du contenu.
