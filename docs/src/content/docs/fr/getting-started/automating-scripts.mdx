...
Une fois que vous avez un script qui vous convient, vous pouvez l'automatiser en utilisant l'[interface en ligne de commande](/genaiscript/reference/cli).

## Exécution d'un script avec la CLI

L'utilisation de base de la CLI est de [lancer](/genaiscript/reference/cli/run/) un script avec un nom d'outil et une liste de fichiers.
...
Consultez la section [Sécurité et confiance](/genaiscript/reference/security-and-trust) pour plus d'informations.
:::
...
Utilisez la [CLI](/genaiscript/reference/cli/run/) pour exécuter le script dans une action GitHub.
...
<Steps>
  <ol>
    ...
    <li>
      Mettez à jour chaque étape qui invoque la [CLI](/genaiscript/reference/cli) pour inclure la variable `AZURE_OPENAI_API_ENDPOINT`.
...
Utilisez l'option `--pull-request-reviews` pour convertir les [annotations](/genaiscript/reference/scripts/annotations) en commentaires de revue **sur le dernier commit** de la demande de tirage (voir [exemple](https://github.com/microsoft/genaiscript/pull/504#pullrequestreview-2093960791)).
