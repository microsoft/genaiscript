...
Pour exécuter un script d'exemple dans le dossier `packages/sample` :

```sh
pnpm run:script <scriptid> ...
```

Dans ce cas, le fichier `packages/sample/.env` sera utilisé pour les variables d'environnement et l'espace de travail sera raciné à `packages/sample`.
