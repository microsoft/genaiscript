...
llmstxt:
  content: >-
    GenAIScript runs in a Node.JS environment with additional globals configured
    by the CLI. To use GenAIScript in a standard Node.JS process, the `run` API
    is required. This API executes GenAIScript scripts in a worker thread,
    ensuring no global pollution or side effects.
    ...
    Environment variables can be set for the GenAIScript process by passing an
    `env` object in the options. By default, the worker inherits `process.env`:


    const results = await run("summarize", ["myfile.txt"], { env: { MY_ENV_VAR:
    "value" } })
  hash: 22922d0ad60a6b0fd8856a1926d42e751c4a23e0917f4dc71023b06dc3c336ee
