...
llmstxt:
  content: >-
    ...
    Bing Web Search. By default, it returns the first 10 search results in the
    `webPages` field as an array, with each entry containing a summary snippet.
    Full web page content can be retrieved using `fetchText`.


    To use Tavily, set the `TAVILY_API_KEY` in the `.env` file. Tavily provides
    a robust search engine API.


    For Bing Web Search, create a Bing Web Search resource in Azure and set the
    `BING_SEARCH_API_KEY` in the `.env` file.
    ...
    ```
  hash: e1be20c8a09349ac816d239202b3c3b15ccecba90d3d4cbb9aac06c1a5dee92b
