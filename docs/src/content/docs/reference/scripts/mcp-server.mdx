...
```sh
genaiscript mcp server --startup load-resources
```
...
- Use `genaiscript mcp inspect` to inspect an MCP server and list the available tools.

```sh
genaiscript mcp inspect npx --yes genaiscript mcp server
```
...
```json title=".vscode/mcp.json"
{
    "servers": {
        "genaiscript": {
            "type": "stdio",
            "command": "npx",
            "args": ["-y", "genaiscript", "mcp", "server", "--cwd", "${workspaceFolder}"],
...
```sh
npx --yes genaiscript mcp server --remote https://github.com/...
```
