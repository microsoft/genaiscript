...
llmstxt:
  content: >-
    Prompt caching reduces processing time and costs for repetitive prompts. The
    `ephemeral` cacheControl setting allows short-term caching by LLM providers.
    ...
    - OpenAI/Azure OpenAI: Automatically enables prompt prefix caching.
    `ephemeral` annotations are ignored.

    - Anthropic: Converts `ephemeral` to `'cache-control': { ... }` in the
    message object. Prompt caching is beta and not supported in older models.
  hash: 6524693f810a582fd144fa09afaadf0052f80aec3c541ae0dddd0218fe987770
