...
For queries that need the current repository context, use the `useCurrentRepo` option to automatically inject `owner` and `name` variables:

```js
// Get repository information with automatic context injection
const repoQuery = `
...
const result = await github.graphql(repoQuery, {}, { useCurrentRepo: true })
...
Combine the `useCurrentRepo` option with additional variables for repository-specific queries:

```js
// Get recent issues and pull requests for current repository
const repoContentQuery = `
...
const data = await github.graphql(repoContentQuery, { count: 5 }, { useCurrentRepo: true })
console.log("Recent issues:", data.repository.issues.nodes)
console.log("Recent PRs:", data.repository.pullRequests.nodes)
```
...
const result = await github.graphql<RepositoryData>(query, variables, { useCurrentRepo: true })
