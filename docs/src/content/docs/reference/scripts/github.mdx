...
You can list and download these artifacts using various methods.
...
```js
const allArtifacts = await github.listArtifacts()
console.log(allArtifacts)
```

**Read artifact by name (finds latest artifact with matching name):**

```js
// Find and download artifact by name
const files = await github.readArtifact("my-build-artifacts")
console.log(files)

// Search within a specific workflow run
const files2 = await github.readArtifact("test-results", { runId: runs[0].id })
console.log(files2)
```
