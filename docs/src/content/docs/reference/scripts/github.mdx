...
llmstxt:
  content: >-
    The `github` module provides helper functions for GitHub queries and
    advanced connection handling. Configuration is auto-detected from the
    environment and git. The `GITHUB_TOKEN` is read from the environment, and
    the issue or pull request number is detected via `GITHUB_ISSUE`. CodeSpaces
    auto-provisions `GITHUB_TOKEN`. In GitHub Actions, permissions like
    `actions: read` and `GITHUB_TOKEN` must be configured.
    ...
    For full GitHub API access, use `octokit`. Example: `const { client } =
    await github.api()`. To work on another repository, use
    `github.client("owner", "repo")`.
  hash: fc7f8990df6d251ab1439f6b5e61ac85886d750ea3c2adc13f5cb5aeca46de32
