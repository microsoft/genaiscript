...
llmstxt:
  content: >-
    The `def` function processes files, including XML. `def("DOCS", env.files)`
    handles all files, while `def("XML", env.files, { endsWith: ".xml" })`
    filters XML files.
    ...
    `const { rss } = XML.parse(await fetch("https://dev.to/feed").then(res =>
    res.text()))`. Extract articles:

    `const articles = items.map(({ title, description }) => ({ title,
    description: parsers.HTMLToText(description) }));`.
  hash: aa6bcf8406a6f84cf903aee40f0766316207372020a8572e1806555a1c1e663f
