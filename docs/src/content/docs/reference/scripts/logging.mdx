...
llmstxt:
  content: >-
    GenAIScript uses the `debug` library for flexible logging. The `env.dbg`
    logger has the `script` namespace and does not send messages to markdown
    traces. To enable logging, use the `DEBUG` environment variable or `--dbg`
    flag with namespace patterns.
    ...
    Wildcards (`*`) allow broader matching, e.g., `DEBUG=connect:*` or exclude
    namespaces with `-`, e.g., `DEBUG=*,-connect:*`.


    In Visual Studio Code, enable "Diagnostics" or set the `DEBUG` setting to a
    specific namespace.
    ...
    GenAIScript internal logs are prefixed with `genaiscript:`, e.g.,
    `DEBUG=genaiscript:*`. Agent logs use `agent:name`.
  hash: 1b9f4560f95c0b7b5fca4d2a129e53aab9d09663d35a567c14973e69da3f7c53
