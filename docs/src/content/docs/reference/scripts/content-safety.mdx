...
throgh the `contentSafety` global object.
...
```js
// detect attach in user prompt
const { attackDetected } = await contentSafety.detectPromptInjection({
    userPrompt: "Hello, world!",
})
// detect attack in documents
const { attackDetected } = await contentSafety.detectPromptInjection({
