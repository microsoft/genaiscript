...
hero:
    image:
        alt:
            A small, flat 8-bit style image displays two rectangular server blocks
...
You can also reference servers by ID from `mcp.json` files using the [system.mcp](#system-mcp) system script.
...
The `system.mcp` system script allows you to reference MCP servers by ID from `mcp.json` files,
following GitHub Copilot Chat conventions.

```js
script({
    system: ["system.mcp"],
    vars: {
        "system.mcp.id": "memory-server", // Reference server by ID
    },
})
```

GenAIScript will automatically resolve the server configuration from:
- `.vscode/mcp.json` (GitHub Copilot Chat format)
- `.ruler/mcp.json` (Ruler format) 
- `mcp.json` (root level)
...
This approach allows centralized MCP server management and consistent configuration
