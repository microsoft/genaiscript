...
-  tool `z3`: Solves a SMTLIB2 problem using the Z3 constraint solver. Send problems one at a time. If the constraint system is in another format, convert the problem to SMTLIB2 format before calling this tool.
...
export default async function (_: ChatGenerationContext) {
    ...
    defTool(
        "z3",
        "Solves a SMTLIB2 problem using the Z3 constraint solver. Send problems one at a time. If the constraint system is in another format, convert the problem to SMTLIB2 format before calling this tool.",
        {
            type: "object",
            properties: {
                smtlib2: {
