...
defTool(
    ...
    async (args) => {
        ...
        let suffix = ""
        if (!pattern && isNaN(count) && res.length > 20) {
            res = res.slice(0, 20)
            suffix =
                "\n...Too many files found. Showing first 20. Use 'count' to specify how many or use 'pattern' to do a grep search..."
        }
...
-  tool `python_code_interpreter_copy_files`: Copy files from the host file system to the container file system. NO absolute paths.
