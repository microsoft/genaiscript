...
llmstxt:
  content: >-
    GenAIScript allows creating AI prompts using Markdown with YAML frontmatter
    for configuration. Scripts use the `.genai.md` extension and consist of YAML
    frontmatter for settings and Markdown content for prompts. YAML options
    include `title`, `description`, `model`, `temperature`, `maxTokens`, and
    advanced settings like `cache`, `system`, and `parameters`. Markdown content
    is converted to a `$` template string in JavaScript, preserving formatting.
    ...
    Summarize the text, focusing on key points, main ideas, and conclusions.
    Limit to 200 words.
    ...
    Markdown scripts are ideal for simple, static prompts, non-technical users,
    and quick prototyping. For complex logic or dynamic content, use
    `.genai.mts` files. Markdown scripts run via CLI, e.g., `genaiscript run
    summarize`. Limitations include no direct JavaScript execution or variable
    interpolation.
  hash: f743bdd466a91b3730d73bad7d9aaad43768f3170cf607da6417bbba9b35183f
...
```markdown title="doc-generator.genai.md"
---
