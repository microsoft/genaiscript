...
GenAIScript are JavaScript files named as `*.genai.mjs`, TypeScript files named as `*.genai.mts`, or Markdown files named as `*.genai.md`,
...
# Text Shortening Script

This script shortens text while preserving meaning.
...
Shorten the provided text. Limit changes to minimum while preserving all important information.
...
-   `.genai.md` are [Markdown files](/genaiscript/reference/scripts/markdown) with YAML frontmatter for configuration and TypeScript code blocks for scripting.
...
- /genaisrc
  ...
  - analyze.genai.md  // markdown script!
  - ...
