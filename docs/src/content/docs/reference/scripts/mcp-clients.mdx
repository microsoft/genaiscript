...
hero:
  image:
    alt: A simple geometric illustration showing a flat icon-style server connected
      to a network node. Next to the server is a small rectangular toolbox with
      a container symbol representing tools or containerized apps. Additional
      details include a small play button or gear symbol, symbolizing operations
      or scripting functionality.
    file: ./mcp-clients.png
llmstxt:
  content: >-
    The Model Context Protocol (MCP) standardizes sharing and consuming tools
    across frameworks and runtimes. GenAIScript provides a lightweight interface
    over the MCP TypeScript SDK, enabling programmatic interaction with MCP
    servers, tool invocation, and resource resolution. While APIs are simpler
    and faster for many tasks, MCPs are tailored for LLM workflows, offering
    containerized, ready-to-use tools that avoid runtime or versioning issues.
    ...
    The server stops automatically after the prompt. Tools can be listed or
    invoked directly:
    ...
    For example, to extract a YouTube video transcript using the
    ...
    MCP servers are not shared between prompts by default but can be reused with
    `host.mcpClient`. Tools can also be defined for reuse across multiple
    prompts.
  hash: 41c874c5955fe716a9e0f0b4286cef7e481e236bfcef5080c5cc625cb0c42818
