...
```mermaid
flowchart TD
    query["query"] -->  |"summarize changes in the current branch"| LLM

    LLM --> |"get changes in current branch"| agent_git["agent git (LLM)"]
    agent_git --> |"diff +main.ts -main.ts...+ new code"| LLM
    agent_git --> git_tools["git branch, git diff"]
    git_tools --> agent_git
```
...
```mermaid
flowchart TD
    query["query"] -->  |"why did the github action 'build' fail?"| LLM

    LLM --> |"query the last failed run"| agent_github["agent github (LLM)"]
    agent_github --> |"commit failed_sha is responsible\nsuccess_sha is last known good"| LLM
    agent_github --> github_tools["list workflow runs, list jobs, diff job logs"]
    github_tools --> agent_github

    LLM --> |"diff success_sha...failed_sha"| agent_git["agent git (LLM)"]
    agent_git --> |"diff +main.ts -main.ts...+ new code"| LLM
    agent_git --> git_tools["git diff"]
    git_tools --> agent_git

    LLM --> response["the change at line 50 is wrong"]
```
...
flowchart TD
    ...
    LLM --> |"query the last failed run"| agent_github["agent github (LLM)"]
    agent_github --> |"run failed_run is responsible"| LLM


    LLM --> |"diff success_sha...failed_run"| agent_git["agent git (LLM)"]
    agent_git --> |"diff +main.ts -main.ts...+ new code"| LLM

    agent_github --> |"remember failed_run, failed_sha"| memory
    agent_git --> |"info about failed_run"| memory["memory (LLM RAG)"]
