...
`logprobs` is a mode where LLMs return probably of each token. 

- See [OpenAI Logprobs](https://cookbook.openai.com/examples/using_logprobs)

:::note

The `logprobs` feature is not available in all model or providers.

:::

## Colored output

When `logprobs` is enable, the [cli](/genaiscript/reference/cli) will color the output based on the probability of each token.
