...
-   add the `disableModelTools` option to the script

```js "disableModelTools: true"
script({
    disableModelTools: true,
})
```

-   or add the `--disable-model-tools` flag to the CLI

```sh "--disable-model-tools"
npx genaiscript run ... --disable-model-tools
```
