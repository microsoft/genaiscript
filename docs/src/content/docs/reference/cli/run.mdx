...
Disables automatic GitHub Actions mode detection. By default, when running in a CI environment (like GitHub Actions),
the CLI automatically skips confirmation prompts for pull request operations. Use this flag to force interactive prompts
even in CI environments.

```sh
npx genaiscript run ... -prd --no-github-actions
```

You can also disable this via the environment variable `GENAISCRIPT_DISABLE_GITHUB_ACTIONS_MODE=true`.

```yaml
- script: npx genaiscript run ... -prd
  env:
    GENAISCRIPT_DISABLE_GITHUB_ACTIONS_MODE: "true"
```
