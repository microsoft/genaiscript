...
The `allowedDomains` property controls which domains are allowed for HTTPS resource resolution via `host.resolveResource()`. 
By default, GitHub domains are allowed for security.
