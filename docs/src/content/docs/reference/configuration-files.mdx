...
or the `GENAISCRIPT_ENV_FILE` environment variable.
...
GenAIScript will collect all `.env` files and merge their content into the final environment variables, starting from config files,
then `GENAISCRIPT_ENV_FILE`, the command line option `--env` values.
