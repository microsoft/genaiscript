...
- `options` - Optional configuration object with:
  - `copyFiles` - List of files to copy from source directory to new worktree (default: [])
...
// Create a worktree with specific files copied and setup steps run
await git.worktreeAdd("/path/to/dev-worktree", "develop", {
    copyFiles: [".env", ".env.local", "config.json"],
    setupSteps: true
})
```
...
- `copyFiles` (optional) - List of files to copy from source directory to new worktree (default: [])
...
// - "Create a development worktree with specific config files and run npm install"
