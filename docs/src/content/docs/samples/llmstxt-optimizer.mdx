...
description: Generate LLM-optimized content stored in markdown frontmatter for better AI processing
sidebar:
  order: 800
llmstxt: "GenAIScript sample for creating LLM-optimized content from markdown files. Processes .md/.mdx files, generates concise versions stored in llmstxt frontmatter field with hash-based caching. Features: individual file processing with runPrompt, intelligent content change detection, 30-50% content reduction while preserving technical accuracy. Usage: genaiscript run llmstxt-optimizer 'docs/**/*.md' --apply-edits. Includes schema extension for Astro Starlight projects, batch processing capabilities, and optimized LLM settings (temperature 0.3, large model)."
llmstxtHash: "a1f2e8c9d4b7a6e3f5c8d1a9b2e7f4c6"
---
