...
llmstxt:
  content: >-
    The `gcm` script generates commit messages for staged Git changes using an
    AI model. It computes a diff, generates a message, and allows the user to
    commit, edit, or regenerate the message. If committed, it can optionally
    push changes.
    ...
    Check for staged changes with `git diff`. If none are staged, the script
    offers to stage all changes:
    ...
    In a loop, generate a commit message based on the diff. Users can edit,
    ...
    Safety measures include system prompts to prevent harmful content, user
    review of messages, and optional use of a content safety filter or service.
  hash: b3b14cb43ffdce75bbb7992406e32419232c0876b031c8504e4f807aae116b8f
