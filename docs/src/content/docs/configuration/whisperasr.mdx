  ...
  transcribe:
    ...
    steps:
      ...
      - name: Run transcription script
        ...
        env:
          WHISPERASR_API_BASE: http://localhost:9000
...
### Alternative: Container in Steps

If you prefer to manage the container manually in your workflow steps:

```yaml title=".github/workflows/transcription-manual.yml"
steps:
  - uses: actions/checkout@v4
  
  - name: Start Whisper ASR container
    run: |
      ...
      timeout 60 bash -c 'until curl -f http://localhost:9000/health; do sleep 2; done'
  ...
  - name: Stop Whisper ASR container
    if: always()
    run: docker stop whisper-asr && docker rm whisper-asr
```

### Security Considerations

When using containers in GitHub Actions:

- Use specific image tags instead of `latest` for reproducible builds
- The whisper-asr service runs locally within the GitHub Actions runner and is not exposed to the internet
- Ensure your audio files don't contain sensitive information, as they will be processed by the containerized service
- Consider using private runners for sensitive workloads
