...
llmstxt:
  content: >-
    Azure AI Search is a content search provider enabling vector search of
    documents. It is configured similarly to other Azure services.
    ...
    **Managed Identity (Entra ID):**
    ...
    1. In the Azure Portal, open your Azure AI Search resource, go to **Overview
    > Properties**.

    2. Enable **Role-based access control** or **Both** in **API Access
    control**.

    3. Assign the **Search Service Contributor** role to your user or service
    principal in **Access Control (IAM)**.


    **API Key:**

    Set `AZURE_AI_SEARCH_ENDPOINT` and `AZURE_AI_SEARCH_API_KEY` in the
    environment:
    ...
    ```
  hash: 66de02b6eb53f2a0d410e73087153904f9184f1e2e294c085b4f83efee0ed87e
