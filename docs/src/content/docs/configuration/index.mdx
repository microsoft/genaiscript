...
llmstxt:
  content: >-
    Configure LLM connections and authorization secrets for GenAIScript to use
    remote (e.g., OpenAI, Azure) or local models (e.g., Ollama, LMStudio). The
    `model` field in the `script` function specifies the model, formatted as
    `provider:model-name`. Use aliases like `small`, `large`, or `vision` for
    default configurations, which can be overridden via CLI, environment
    ...
    `.env` files store secrets and configurations. Add `.env` to `.gitignore` to
    prevent committing sensitive data. Default `.env` file loading order:
    `~/.env.genaiscript`, `./.env.genaiscript`, `./.env`. Specify custom `.env`
    locations via CLI:
    ...
    Use `npx genaiscript configure` to interactively configure and validate LLM
    connections.
    ...
    Custom OpenAI-compatible providers require environment variables like
    `_API_BASE` and `_API_KEY`:
    ...
    ```
  hash: 3b49683930a67dcba019ff997bb8235f17191d00c7eb3ae5beb2552abd705a8e
