...
\` becomes ', extra isWhiteSpaceSingleLine, `logic` becomes `logique`, and so forth.
...
- Do not translate snippets like \`\`<text>\`\` 
- Do not translate snippets like \`||variables:<text>||\` 
- Do not translate markdown anchors like \`#anchor\`
`
````
...
```sh
node .genaiscript/genaiscript.cjs run translate ... --vars lang=French
```