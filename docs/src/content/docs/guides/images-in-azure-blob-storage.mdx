...
llmstxt:
  content: >-
    Use the Azure Node.js SDK to download images from Azure Blob Storage and
    include them in prompts. The `defImages` function supports Node.js `Buffer`.
    ...
    Connect to Azure Blob Storage using `BlobServiceClient` and
    `DefaultAzureCredential`. Extract `account` and `container` from `env.vars`:
    ...
    Summarize results by storing image descriptions with `def` and prompting for
    a summary:
    ...
    ```
  hash: b22df75b68678f92d557474d4d1266f27f380f23a1fc43506fb9782cbf6df60f
