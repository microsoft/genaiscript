...
llmstxt:
  content: >-
    The `retrieval` APIs now support Azure AI Search, enabling indexing of files
    as embeddings in a vector database for similarity search, commonly used in
    ...
    GenAIScript simplifies interaction with Azure AI Search by handling
    chunking, vectorization, and indexing. The `retrieval.index` function
    creates an index, `insertOrUpdate` indexes files, and `search` retrieves
    relevant matches. Files can also be pre-indexed via the command line.
  hash: 04edd77115b9e6029dda40aabaa77f66d2ec4e3e081872d34680ecdae787f043
